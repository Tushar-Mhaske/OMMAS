...
function ListPDFFiles(BlockCode) {
    ...
    jQuery("#tbCSVFilesList").jqGrid({
        url: '/ExistingRoads/ListHabMultipleCSVFiles',
        datatype: "json",
        mtype: "POST",
        ...
        colModel: [
                    ...
                    { name: 'Finalise', index: 'Finalised', width: 60, sortable: false, align: "center", editable: false  },
        ],
...
function AnchorFormatter(cellvalue, options, rowObject) {
    var url = "/ExistingRoads/DownloadHabMultipleCSVFile/" + cellvalue;
...
// FinaliseHabCSVDetails



function FinaliseHabCSVDetails(id) {
    if (confirm("Are you sure to finalize the CSV and it's details ? ")) {

        $.ajax({
            url: "/ExistingRoads/FinaliseHabCSV?id="+id,
            type: "POST",
            cache: false,
            data: {
            ...
            success: function (response) {
                ...
                if (response.Success) {
                    ...
                    alert("CSV File and it's Details Finalized Succesfully.");
                    closeMonitorsInspectionDetails();
...
function DeleteHabCSVFileDetails(id) {
    if (confirm("Are you sure to delete the CSV and it's details ? ")) {

        $.ajax({
            url: "/ExistingRoads/DeleteHabCSVFileDetails/",
            type: "POST",
            cache: false,
            data: {
                FileID: id, value: Math.random()
            ...
            success: function (response) {
                ...
                if (response.Success) {
                    ...
                    $("#NumberofPdfs").val(parseFloat($("#NumberofFiles").val()) - 1);
                    alert("CSV File and it's Details Deleted Succesfully.");
                    closeMonitorsInspectionDetails();
                    $("#tbPDFFilesList").trigger('reloadGrid');
                    $('#tblTraceMapList').trigger('reloadGrid');
                }
...
{
    $.ajax({
        url: "/ExistingRoads/GetHabCSVValue/",
        type: "POST",
        cache: false,
        data: {
