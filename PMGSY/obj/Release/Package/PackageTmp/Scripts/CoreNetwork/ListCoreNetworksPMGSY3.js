...
$(document).ready(function () {
    ...
    $("#ddlBlocks").change(function () {
        ...
        CheckTraceMapIsFinalizedOrNotIn();
    });
    ...
    $("#btnAddCoreNetwork1").click(function () {
        if ($('#RoleCode').val() == 25) {
            if (parseInt($("#ddlStates option:selected").val()) <= 0) {
                alert('Please select state');
                return false;
...
function LoadNetworkGrid() {
    ...
    jQuery("#networkCategory").jqGrid({
        url: '/CoreNetwork/GetCoreNetWorksListPMGSY3',
        datatype: "json",
        mtype: "GET",
        ...
        colNames: ['System Id', 'Route Type', 'Road No.', 'Road Name', 'DRRP Road Code', 'Road From', 'Road To', 'Start Chainage [In Km]', 'End Chainage [In Km]', 'Length [In Km]', 'TR/MRL Road Length [In Km]', 'Habitation', 'Upload', 'Map DRRP Road', 'Map InterDistrict Habitation', 'View', 'Edit', 'Delete'],
        colModel: [
                            { name: 'CNCode', index: 'HabitationCode', height: 'auto', width: 60, align: "left", sortable: true, search: true },
                            ...
                            { name: 'PLAN_RD_TOTAL_LENGTH', index: 'PLAN_RD_TOTAL_LENGTH', height: 'auto', width: 70, align: "center", search: false, hidden: (scheme == 1 ? true : false) },
                            { name: 'Habitation', index: 'Habitation', height: 'auto', width: 70, align: "left", sortable: false, search: false, hidden: ($("#RoleCode").val() == 22 ? false : true) },
                            { name: 'c', width: 50, sortable: false, resize: false, align: "center", search: false, hidden: true /*hidden: ($("#RoleCode").val() == 22 ? false : true)*/ },
                            { name: 'mapRoad', width: 60, sortable: false, resize: false, align: "center", search: false, hidden: ($("#RoleCode").val() == 22 ? (scheme == 1 ? true : false) : true) },
                            { name: 'mapInterDistrcthabitation', index: 'mapHabitation', width: 70, align: "center", search: false, hidden: ($("#RoleCode").val() == 22 ? ($("#PMGSYScheme").val() == 4 ? false : true) : true), sortable: false },
                            { name: 'Details', index: 'Details', height: 'auto', width: 50, align: "left", sortable: false, search: false },
                            { name: 'a', width: 50, sortable: false, resize: false, align: "center", search: false, hidden: (($("#RoleCode").val() == 22 || $("#RoleCode").val() == 25) ? false : true) },
                            { name: 'b', width: 50, sortable: false, resize: false, align: "center", search: false, hidden: (($("#RoleCode").val() == 22 || $("#RoleCode").val() == 25) ? false : true) }
        ],
...
function mapInterDistrictHabitationDetails(urlparameter) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddForm").load('/CoreNetWork/ListHabitationsInterDistrictHabitationPMGSY3/' + urlparameter, function (data) {
            unblockPage();
            if (data.success == false) {
                alert("Error occurred while processing the request.");
            }
...
function searchDetails() {
    ...
    $('#networkCategory').setGridParam({
        url: '/CoreNetwork/GetCoreNetWorksListPMGSY3', datatype: 'json'
    });
...
function MapOtherCandidateRoad(urlparameter) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddForm").load('/CoreNetWork/MapOtherCandidateRoadViewPMGSY3/' + urlparameter, function (data) {
            unblockPage();
            if (data.success == false) {
                alert("Error occurred while processing the request.");
            }
...
function CheckIfBlockFinalized()
{
    $.ajax({
        type: 'POST',
        url: '/CoreNetwork/CheckLockStatus/',
        async: false,
        cache: false,
        ...
        success: function (data) {
            ...
            {// Locked
             //   alert("Stratus 1:" + data.status)

                $('#btnAddCoreNetwork').hide('slow');
                $('#btnAddCN').hide('slow');
                
            }
            else {// Unlocked
              //  alert("Stratus 2:" + data.status)

               // $('#btnAddCoreNetwork').hide('slow');
                $('#btnAddCN').show('slow');
                $('#btnAddCoreNetwork').hide('slow');
               // $('#btnAddCoreNetwork').show('slow');
            }
        }
    });
...
//-->  1) If Trace Map is finalized then show Add Button. 
//2) If TR / MRL is finalized then hide Add Button (select * from omms.MAST_CN_BLOCK_PMGSY3_FINALIZE -- Block Level TR / MRL Finalize)
//3) If TR / MRL is finalized and Not Unlocked then hide Add Button
//--->  4)  If TR / MRL is finalized and  Unlocked then show Add Button
//--->	5) If Unlocked (omms.IMS_UNLOCK_DETAILS table entry for given time) then Show Add Button 

//*@
function CheckTraceMapIsFinalizedOrNotIn() {
   // alert("S")

    $.ajax({
        type: 'POST',
        url: '/CoreNetwork/CheckTraceMapIsFinalizedOrNot/',
        async: false,
        cache: false,
        data: { blockCode: $('#ddlBlocks option:selected').val(), },
        success: function (data) {
            if (data.status == true) {// Locked
                //   alert("Stratus 1:" + data.status)
