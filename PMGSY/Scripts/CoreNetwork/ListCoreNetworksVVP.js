...
$(document).ready(function () {
    scheme = $("#Scheme").val();
    title = scheme == 1 ? "Core Network" : "Candidate Road";
    ...
    $("#btnAddCoreNetwork").click(function () {
        ...
        $('#accordion').show('fold', function () {
            blockPage();
            $("#divAddForm").load("/CoreNetwork/AddEditCoreNetworksVVP/" + $('#ddlStates').val() + "$" + $('#ddlDistricts').val() + "$" + blockCode, function () {
                $.validator.unobtrusive.parse($('#frmAddCoreNetwork'));
                unblockPage();
            });
...
function LoadNetworkGrid() {
    ...
    jQuery("#networkCategory").jqGrid({
        url: '/CoreNetwork/GetCoreNetWorksListVVP',
        datatype: "json",
        mtype: "POST",
        ...
        colNames: ['Core Network System Id', 'Route Type', 'Road No.', 'Road Name', 'DRRP Road Code', 'Road From', 'Road To', 'Start Chainage [In Km]', 'End Chainage [In Km]', 'Length [In Km]', 'Candidate Road Length [In Km]', 'Habitation', 'Upload', 'Map DRRP Road', 'View', 'Edit', 'Delete'],
        colModel: [
            ...
            { name: 'c', width: 50, sortable: false, resize: false, align: "center", search: false, hidden: true /*($("#RoleCode").val() == 22 ? false : true)*/ },
        ...
        caption: scheme == 1 ? "&nbsp;&nbsp; Core Network List" : "&nbsp;&nbsp; Candidate Road List",
        height: 'auto',
        autowidth: true,
        hidegrid: true,
        rownumbers: true,
...
function editNetworkData(urlparameter) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddForm").load('/CoreNetWork/EditCoreNetworksVVP/' + urlparameter, function (data) {
            unblockPage();
...
function deleteNetworkData(urlparameter) {
    if (confirm("Are you sure you want to delete Core Network Details?")) {
        $.ajax({
            type: 'POST',
            url: '/CoreNetWork/DeleteCoreNetwork/' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
            success: function (data) {
                if (data.success) {
                    alert("Core Network details deleted successfully ");
                    $("#networkCategory").trigger('reloadGrid');
                    //$('#divAddForm').show();
                }
                else {
                    alert("Core Network details can not be deleted as [Habitation] or [File details] or [Feedback] are added to this road or Core Network is used in Proposal or PCI Index is entered on this Road.");
                }
            },
...
function editHabitationDetails(urlparameter) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddForm").load('/CoreNetWork/ListHabitationsVVP/' + urlparameter, function (data) {
            unblockPage();
            if (data.success == false) {
                alert("Error occurred while processing the request.");
            }
...
function detailsCoreNetwork(urlparameter) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddForm").load('/CoreNetWork/DetailsCoreNetworkVVP/' + urlparameter, function (data) {
            unblockPage();
            if (data.success == false) {
                alert("Error occurred while processing the request.");
            }
...
function searchDetails() {
    ...
    $('#networkCategory').setGridParam({
        url: '/CoreNetwork/GetCoreNetWorksListVVP', datatype: 'json'
    });
...
function MapOtherCandidateRoad(urlparameter) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddForm").load('/CoreNetWork/MapOtherCandidateRoadViewVVP/' + urlparameter, function (data) {
            unblockPage();
            if (data.success == false) {
                alert("Error occurred while processing the request.");
            }
