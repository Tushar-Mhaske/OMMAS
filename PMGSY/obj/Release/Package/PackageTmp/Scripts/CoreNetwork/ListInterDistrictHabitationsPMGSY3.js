...
$(document).ready(function () {
    ...
    LoadHabitationstoMapListInterDistrictHabitationScheme2( )
    ...
    $('#ddlRoadsToMapInterDistricthabitation').change(function () {
        if ($(this).val() > -1) {
            LoadHabitationstoMapListInterDistrictHabitationScheme2( );
        }
    });
...
function LoadHabitationstoMapListInterDistrictHabitationScheme2() {
  
 
   jQuery("#mapHabitationCandidateList").jqGrid('GridUnload');
        jQuery("#mapHabitationCandidateList").jqGrid({
            url: '/CoreNetwork/GetHabitationListToMapInterDistrictHabitationPMGSY3',
            datatype: "json",
            mtype: "GET",
            ...
            postData: { habCode: $('#EncryptedRoadCode').val(), erRoadCode: $('#ddlRoadsToMapInterDistricthabitation option:selected').val(), habDirect: ($('#rdbHabDirectY').is(':checked') ? 'Y' : 'N') },
...
$(document).ready(function () {
    ...
    $('#ddldistrictsToMap').change(function () {
       
        $("#ddlblocks").empty();
        $("#ddlRoadsToMapInterDistricthabitation").empty();
        LoadHabitationstoMapListInterDistrictHabitationScheme2( );
      $("#ddlRoadsToMapInterDistricthabitation").empty();
        $.ajax({
            url: '/CoreNetwork/PopulateBlocksbyDistrictCode',
            type: 'POST',
            async: false,
            cache: false,
            beforeSend: function () {
            ...
            data: { districtCode: $("#ddldistrictsToMap").val(), },
            success: function (jsonData) {
                for (var i = 0; i < jsonData.length; i++) {
...
$(document).ready(function () {
    ...
    $('#ddlblocks').change(function () {
        ...
        $.ajax({
            url: '/CoreNetwork/PopulateRoadsbyBlockCode',
            type: 'POST',
            async: false,
            cache: false,
            beforeSend: function () {
            ...
            data: { blockCode: $("#ddlblocks").val(), districtCode: $("#ddldistrictsToMap").val(), },
            success: function (jsonData) {
                ...
                for (var i = 0; i < jsonData.length; i++) {
                    ...
                    $("#ddlRoadsToMapInterDistricthabitation").append("<option value='" + jsonData[i].Value + "'>" + jsonData[i].Text + "</option>");
