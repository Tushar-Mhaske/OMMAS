$(document).ready(function () {
    ...
    GetTrafficIntensityPMGSY3($("#MAST_ER_ROAD_CODE").val());
    ...
    $('#btnSave').click(function (evt) {
        ...
        if ($('#frmTrafficIntensity').valid()) {
            if (validateForm() == true) {
                ...
                $.ajax({
                    url: "/ExistingRoads/AddTrafficIntensityPMGSY3/",
                    type: "POST",
                    cache: false,
    ...
    $('#btnUpdate').click(function (evt) {
        ...
        if ($('#frmTrafficIntensity').valid()) {
            $.ajax({
                url: "/ExistingRoads/EditTrafficIntensityPMGSY3/",
                type: "POST",
                cache: false,
                data: $("#frmTrafficIntensity").serialize(),
                beforeSend: function () {
    ...
    $('#btnCancel').click(function () {
        ...
        $('#accordion').show('fold', function () {
            ...
            $("#divExistingRoadsForm").load('/ExistingRoads/ListTrafficIntensityPMGSY3/' + $('#EncryptedErRoadCode').val(), function () {
...
function PopulateTrafficIntensityYears(MAST_ER_ROAD_CODE) {
    ...
    $.ajax({
        url: '/ExistingRoads/PopulateTrafficIntensityYearsPMGSY3/',
        type: 'POST',
        beforeSend: function () {
            blockPage();
        },
...
function GetTrafficIntensityPMGSY3(MAST_ER_ROAD_CODE) {
...
function EditTrafficIntensity(key) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divExistingRoadsForm").load('/ExistingRoads/ListTrafficIntensityPMGSY3/' + key, function () {
            $.validator.unobtrusive.parse($('#frmTrafficIntensity'));
            if ($("#Operation").val() == "U") {
                $("#MAST_TOTAL_TI").focus();
            }
...
function DeleteTrafficI(key) {
    ...
    if (confirm("Are you sure you want to delete the traffic intensity details ? ")) {
        ...
        $.ajax({
            url: "/ExistingRoads/DeleteTrafficIntensityPMGSY3/" + key,
            type: "POST",
            cache: false,
            //data: { IMS_PR_ROAD_CODE: IMS_PR_ROAD_CODE, IMS_TI_YEAR: IMS_TI_YEAR, value: Math.random() },
            data: { __RequestVerificationToken: $("#frmTrafficIntensity input[name=__RequestVerificationToken]").val() },
            beforeSend: function () {
                blockPage();
            },
            error: function (xhr, status, error) {
