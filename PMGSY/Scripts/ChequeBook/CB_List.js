        ...
        if (isGridLoaded)
        {
            ...
            } else
            {
               // LoadGrid($("#ddlMonth").val(), $("#ddlYear").val(), $("#txtChequeNo").val(), $("#ddlSearchDPIU").val(), $("#ddlSearchDPIU option:selected").text());
                LoadGrid($("#ddlMonth").val(), $("#ddlYear").val(), $("#txtChequeNo").val(), $("#ddlSearchDPIU").val(), "D", $("#lstAccType").val());
            }
        }
        else
        {
...
function LoadGrid(month, year, cheque, adminNdCode, dpiuName, AccountType) {
    if (cheque == "") {
        cheque = 0;
    }
   ...
   jQuery("#tblChequeBookList").jqGrid({
        ...
        datatype: "json",
        mtype: "POST",
       colNames: ['Cheque Book Received Date', 'Cheque Book Type', 'Start Number', 'End Number', 'Edit', 'Delete'],
        colModel: [
                            { name: 'ISSUE_DATE', index: 'ISSUE_DATE', width: 100, align: 'center', sortable: true },
                            { name: 'BOOK_TYPE', index: 'BOOK_TYPE', width: 40, align: 'center', sortable: true },
