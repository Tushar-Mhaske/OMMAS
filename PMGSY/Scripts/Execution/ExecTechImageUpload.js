...
$(function () {
    ...
    $('#fileupload').bind('fileuploadsubmit', function (e, data) {
        ...
        $("#tblPresentation textarea[name='remark[]']").each(function () {
            ...
            if ($('#txtRemarks').val() != '') {
                if (!regExRemarks.test($('#txtRemarks').val())) {
                    $("#divError").html("Please Enter Valid Remarks,Can only contains AlphaNumeric values and [,.()-]. value");
                    $("#divError").show('slow');
                    flagRemarks = false;
                    return false;
                }
...
this.imagePreview = function () {
    ...
    $("a.preview").hover(function (e) {
        ...
        $("body").append("<p id='preview'><img  style='height: 500px; width: 500px;' height='800' width='600' src='" + this.href + "' alt='Image Not    ' />" + c + "</p>");
        callback(e, 200);
        $("#preview").fadeIn("slow");
    },
		function () {
...
function ListProposalFiles(IMS_PR_ROAD_CODE) {
    ...
    jQuery("#tbFilesList").jqGrid({
        ...
        colModel: [
                    ...
                    { name: 'Description', index: 'Description', width: 400, sortable: false, align: "center", editable: true, editoptions: { maxlength: 255 }, editrules: { custom: true, custom_func: ValidateImageDescription } },
                    { name: 'Stage', index: 'Stage', width: 300, sortable: false, align: "center", search: false, editable: false },
                    { name: 'Remarks', index: 'Remarks', width: 200, sortable: false, align: "center", editable: true, editoptions: { maxlength: 255 }, editrules: { custom: true, custom_func: ValidateImageRemarks } },
...
function ValidateImageRemarks(value, colname) {
    //if (!value.match("^[a-zA-Z0-9 ]+$")) {
    //    return [" Invalid Remarks Only Alphabets and Numbers are allowed."];
    //}
    //else {
    //    return [true, ""];
    //}
...
function DeleteFileDetails(param, IMS_FILE_NAME) {
    ...
    if (confirm("Are you sure to delete the Image and Image details ? ")) {
        ...
        $.ajax({
            url: "/Execution/DeleteExecTechFileDetails/",
            type: "POST",
            cache: false,
            data: {
