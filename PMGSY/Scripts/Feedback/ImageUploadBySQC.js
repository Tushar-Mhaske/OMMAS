...
$(function () {
    ...
    $('#fileupload').bind('fileuploadadd', function (e, data) {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: '/FeedbackDetails/ReadGeoPositions/',
            data: data.files[0],
            cache: false,
            processData: false,
            contentType: false,
            success: function (data) {
                if (data.success == true) {
                    ...
                    //  alert("3")
                    $("#btnCancelImageUpload").trigger('click');
                    alert(data.message);
                    $('#dvErrorMessage').show('slow');
                    $('#rdoImage').trigger('click');

                    // btnCancelImageUpload
...
function ListQualityFiles(FEED_ID,REP_ID) {
  ...
  //  alert("Hre")
    ...
    jQuery("#tbFilesList").jqGrid({
        url: '/FeedbackDetails/ListFilesBySQC?' + Math.random(),
        datatype: "json",
        mtype: "POST",
        colNames: ["Image", 'Uploaded By','Description of Image',"Download", "Delete Image"],
        colModel: [
                   ...
                   //{ name: 'Description', index: 'Description', width: 200, sortable: false, align: "left", editable: true, editoptions: { maxlength: 255 }, editrules: { custom: true, custom_func: ValidateImageDescription } },
                    { name: 'a', index: 'a', width: 60, sortable: false, align: 'center', editable: false, hidden: isHiddenImage(), title: false }, // Download
                    { name: 'b', index: 'b', width: 60, sortable: false, align: "center", editable: false, hidden: isHidden(FinalizedByPIU), title: false } // Delete
...
function DownLoadImageSQC(cellvalue) {
    //alert("cellvalue = " + cellvalue)
    var url = "/FeedbackDetails/DownloadFileUploadedBySQC/" + cellvalue;
...
function DeleteFileDetailsSQC(cellvalue) {

    if (confirm("Are you sure to delete the Image and Image details ? ")) {

        $.ajax({
            url: "/FeedbackDetails/DeleteFileDetailsBySQC/" + cellvalue,
            type: "POST",
            cache: false,
            //data: {
...
$('#SameImageAsPIU').click(function () {

    SameImageAsPIUDetails();
});

function SameImageAsPIUDetails() {

    if (confirm("Are you sure to upload same Image as PIU ? ")) {

        $.ajax({
            url: "/FeedbackDetails/UploadSameImageAsPIUDetails/",
            type: "POST",
            cache: false,
            data: {
            ...
            success: function (response) {
                ...
                $("#tbFilesList").trigger('reloadGrid');
                if (response.Success) {
                    alert("Image details uploaded Succesfully.");
                    ListQualityFiles($("#FEED_ID").val(), $("#REP_ID").val());
                }
                else {
                    alert(response.ErrorMessage);
