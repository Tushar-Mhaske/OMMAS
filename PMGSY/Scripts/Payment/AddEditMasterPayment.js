...
$(document).ready(function () {
    ...
    $('#Cheque').on('change', function () {
        ...
        if ($(this).is(':checked')) {
            ...
            //If Condition Added By Abhishek kamble 11-Apr-2014 start  . Condition for txn_id=3185 , 3187 , 3173 added on 05-05-2023
            if (($("#TXN_ID option:selected").val() != "47$Q") && ($("#TXN_ID option:selected").val() != "737$Q") && ($("#TXN_ID option:selected").val() != "1484$Q") || ($("#TXN_ID option:selected").val() == "1974$Q") || ($("#TXN_ID option:selected").val() == "3185$Q") || ($("#TXN_ID option:selected").val() == "3187$Q") || ($("#TXN_ID option:selected").val() == "3173$Q")) {//PMGSY3
    ...
    $('#Advice').on('change', function () {
        ...
        //if ($("#fundType").val() == "A" && ($("#TXN_ID option:selected").val() == "472$Q" || $("#TXN_ID option:selected").val() == "390$Q")&& $('#btnSubmit').data('clicked') != true) {
        //    ($("#TXN_ID").trigger("change"));//Added on 16-02-2023
        //}
        triggerTxnChange();
    })

    //Below code added on 14-06-2023
    function triggerTxnChange() {
        //alert("Submit button clicked or not " + $('#btnSubmit').data('clicked'))
    ...
    $("#TXN_ID").change(function () {
        ...
        //call to set value balanceCashAmount,balancechqamount for sda and holding account


        getPaymentClosingBalance($("#BILL_MONTH").val(), $("#BILL_YEAR").val());
        //alert("balanceCashAmount :" + balanceCashAmount);
        //alert("balanceChequeAmount :" + balanceChequeAmount);
        ...
        $.ajax({
            ...
            //data: { billDate: $('#BILL_DATE').val() },//Added on 01-07-2022
            data: { billDate: $('#BILL_DATE').val(), isChqChecked: $('#Cheque').is(":checked") ? "Y" : "N", isAdvChecked: $('#Advice').is(":checked") ? "Y" : "N" },//Added on 13-06-2023
            async: false,
            // data: $("#authSigForm").serialize(),
            error: function (xhr, status, error) {
                unblockPage();
            ...
            success: function (data) {
                ...
                if (data != "") {
                    //Conditions to check Security Deposit and Holding Account Bank Validation
                    $('#HoldBankVal1').hide();
                    $('#HoldBankVal2').hide();
                    $('#HoldBankVal3').hide();
                    $('#SecDepBankVal1').hide();
                    ...
                    if (data == -1) {
                        alert("Holding Bank Account details is not available.");
                        $("#TXN_ID").val(0);
                        resetMasterForm();
                        $('#FormParameters').hide();
                        ...
                        alert("Holding Bank Account Number is not available.");
                        $("#TXN_ID").val(0);
                        resetMasterForm();
                        $('#FormParameters').hide();
                        $('#PFMSValidations').show();
                        ...
                        alert("Holding Bank IFSC Code is not available.");
                        $("#TXN_ID").val(0);
                        resetMasterForm();
                        $('#FormParameters').hide();
                        $('#PFMSValidations').show();
                        ...
                        alert("Security Deposit Bank Account details is not available.");
                        $("#TXN_ID").val(0);
                        resetMasterForm();
                        $('#FormParameters').hide();
                        $('#PFMSValidations').show();
                        ...
                        alert("Security Deposit Bank IFSC Code is not available.");
                        $("#TXN_ID").val(0);
                        resetMasterForm();
                        $('#FormParameters').hide();
                        $('#PFMSValidations').show();
                        ...
                        alert("Security Deposit Bank Account Number is not available.");
                        $("#TXN_ID").val(0);
                        resetMasterForm();
                        $('#FormParameters').hide();
                        $('#PFMSValidations').show();
                    ...
                    if (data.CashOrCheque == "Q") {
                        ...
                        //Below condition commented on 14-06-2023
                        //$('#Cheque').trigger('click');
                       
                        //Below condition added on 14-06-2023
                        if (($("#TXN_ID option:selected").val() == "472$Q" || $("#TXN_ID option:selected").val() == "390$Q") && $("#fundType").val() == "A") {

                            if ($('#Epay').is(":checked")) {

                                $('#Epay').trigger('click');

                            } else if ($('#Advice').is(":checked")) {
                        ...
                        $("#divCheque").hide();//Added on 04-05-2023
                    }
                    ...
                    //Below condition added on 28-01-2023 to disable deductions
                    //if (data.IsDedBypass == "Y") {
                    ...
                    if (data.EpayRequired == "N") {
                        ...
                        //alert("Txn_Id :" + $("#TXN_ID option:selected").val());

                        $("#divEpay").show();
                        //Below condition added on 04-05-2023
                        if ($("#TXN_ID option:selected").val() == "3187$Q" || $("#TXN_ID option:selected").val() == "3185$Q") {
                            //alert("Trigger");
                            $('#Epay').click();
                        }
                    }
        ...
        if (($("#TXN_ID option:selected").val() == "3173$Q")) {
            $('#divERemit').hide();
        }
    });
    ...
    $("#btnSubmit").click(function (e) {
        ...
        $(this).data('clicked', true);//Added on 13-06-2023
...
function getPaymentClosingBalance(month, year) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#errorSpan').html("");
            if (data != "") {
                //alert("transaction for closing balance :3826" + $("#TXN_ID option:selected").val());
                //Below Condition Added on 05-05-2023
                if ($("#TXN_ID option:selected").val() == "3187$Q") {
                    //alert("security"+3187);
                    balanceCashAmount = data.Cash;
                    balanceChequeAmount = data.BankAuthSec;
                } else if ($("#TXN_ID option:selected").val() == "3185$Q" || $("#TXN_ID option:selected").val() == "3173$Q") {
                    //alert("Holding" + (3185 , 3173));
                    balanceCashAmount = data.Cash;
                    balanceChequeAmount = data.BankAuthHold;
                } else {
                    balanceCashAmount = data.Cash;
                    balanceChequeAmount = data.BankAuth;
                }
                //Below lines commeneted on 05-05-2023
                //balanceCashAmount = data.Cash;
                //balanceChequeAmount = data.BankAuth;
                ...
                //Below lines added on 05-05-2023
                $("#lblBankHold").text(parseFloat(data.BankAuthHold).toFixed(2));

                $("#lblBankSec").text(parseFloat(data.BankAuthSec).toFixed(2));
            }
        }
    });
