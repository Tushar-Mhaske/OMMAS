$(document).ready(function () {
    ...
    LoadAddView();
    $.validator.unobtrusive.parse("divAddCdWorks");
    GetCdWorksPMGSY3($("#ExistingRoadCode").val());

});
function GetCdWorksPMGSY3(MAST_ER_ROAD_CODE) {
...
function EditCdWorks(key) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddCdWorks").load('/ExistingRoads/CdWorkPMGSY3Layout?id=' + key, function () {
...
function DeleteCdWork(key) {
    ...
    if (confirm("Are you sure you want to delete the CD Works details ? ")) {
        ...
        $.ajax({
            url: "/ExistingRoads/DeleteCDWorksDetailsPMGSY3/" + key,
            type: "POST",
            cache: false,
            data: { __RequestVerificationToken: $("#frmListCDWorksPMGSY3 input[name=__RequestVerificationToken]").val() },
            beforeSend: function () {
                blockPage();
            },
...
function LoadAddView() {
    $.ajax({
        ...
        type: 'GET',
        url: '/ExistingRoads/CdWorkPMGSY3Layout/',
