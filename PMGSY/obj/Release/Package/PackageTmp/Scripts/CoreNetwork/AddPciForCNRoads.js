...
function GetPCIListForCNRoad() {
 ...
 //   alert("here");
    ...
    jQuery("#tbPciForCNRoad").jqGrid({
        url: '/CoreNetwork/GetPCIListForCNRoad',
        datatype: "json",
        mtype: "POST",
        ...
        colModel: [
                    ...
                    { name: 'delete', index: 'delete', width: 120, sortable: false, align: "center" },
                    { name: 'upload', index: 'upload', width: 90, sortable: false, align: "center" },
        ],
...
function UploadChainagePhotoGraph(id) {
    ...
    $.ajax({
        url: '/CoreNetwork/GetPhotoUploadView/' + id,
        type: "POST",
        cache: false,
        async: false,
        success: function (response) {
...
$("#MANE_PCI_YEAR").change(function () {
   ...
   // alert("Year Changed 2")
    ...
    $.ajax({
        url: '/CoreNetwork/GetCNRoadLengthDetailsDetailsForVerification',
        type: "POST",
        cache: false,
        data: { ENC_PLAN_CN_ROAD_CODE: $("#ENC_PLAN_CN_ROAD_CODE").val(), MANE_IMS_YEAR: $("#MANE_PCI_YEAR").val(), ER_ROAD_CODE: $("#ER_ROAD_CODE").val(), value: Math.random() },
        beforeSend: function () {
            blockPage();
        },
        error: function (xhr, status, error) {
        ...
        success: function (response) {
            ...
            {
                unblockPage();
                $("#MANE_STR_CHAIN").val(response.MANE_ST_CHAIN);
            }
            else
            {
                unblockPage();
    ...
    $.ajax({
        url: '/CoreNetwork/CheckIfDRRPIsFinalizedOrNot',
        type: "POST",
        cache: false,
        data: { ENC_PLAN_CN_ROAD_CODE: $("#ENC_PLAN_CN_ROAD_CODE").val(), MANE_IMS_YEAR: $("#MANE_PCI_YEAR").val(), ER_ROAD_CODE: $("#ER_ROAD_CODE").val(), value: Math.random() },
        beforeSend: function () {
        ...
        success: function (response) {
            ...
            if (response.Success)
            {
                ...
                if (!(response.IS_FINALIZED || response.IS_LEN_COMPLETED))
                {
                    $("#rowAdd").show();
