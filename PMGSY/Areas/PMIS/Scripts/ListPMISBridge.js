...
function LoadPMISBridgeList() {
    ...
    jQuery("#tbPMISBridgeList").jqGrid({
        ...
        datatype: "json",
        mtype: "POST",
        colNames: ['State', 'District', 'Block', 'Package Name', 'Sanction Year', 'Sanction Date', 'Batch', 'Length(in Km)', 'Agreement Number', 'Agreement Cost', 'MoRD Share', 'State Share', 'Total Sanctioned Cost', 'Bridge Name', 'Add/Update Project Plan', 'Finalize', 'Revise Plan Details', 'Add/Update Actuals'], //, 'Revise Plan Details' 
        colModel: [
                         ...
                         { name: 'ACTUALS', width: 120, resize: false, align: "center", hidden: false }
                  //       { name: 'CHAINAGE', width: 120, resize: false, align: "center", hidden: false }// formatter: FormatActualsColumn,
    ...
    if (confirm("Are you sure to revise Plan ?")) {
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    LoadPMISBridgeList();
                    EditProjectPlan(RoadCode, status);
...
function AddActuals(cellvalue) {
    ...
    $('#accordion').show('slow', function () {
        ...
        $("#divPMISBridgeListForm").load('/PMIS/AddBridgeActuals/' + cellvalue, function () {
            $.validator.unobtrusive.parse($('#divPMISBridgeListForm'));
