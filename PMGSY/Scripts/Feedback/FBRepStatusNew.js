...
function loadFBRepStatus() {
    $("#tbFBReplyStatusJqGrid").jqGrid({
        ...
        colNames: ["Reply Id", "Reply Date", "Comment", 'Reply By', "Status", "ETA (Tentative Timeline to resolve the complaint)", "Was any action taken to address the complaint?", 'Image Upload / View', 'PDF Upload / View', ($("#RoleCode").val() == 22 || $("#RoleCode").val() == 38 || $("#RoleCode").val() == 54) ? 'Forward to SQC' : 'Reply Stage', "Edit", "Delete", 'Add Reply ( SQC )', 'Delete', 'Send Reply to complainer'],
        colModel: [
            ...
            { name: 'd', width: 120, sortable: false, resize: false, align: "center", search: false, hidden: (($("#RoleCode").val() == 22) || ($("#RoleCode").val() == 38) || ($("#RoleCode").val() == 54) || ($("#RoleCode").val() == 25) ? true : false) }, // Add Details by SQC - AddReplayBySQC     hidden: ($("#RoleCode").val() == 22 ? true : false)

            { name: 'f', width: 120, sortable: false, resize: false, align: "center", search: false, hidden: (($("#RoleCode").val() == 22) || ($("#RoleCode").val() == 38) || ($("#RoleCode").val() == 54) || ($("#RoleCode").val() == 25) ? true : false) }, //  DelBySQC  -- Deletion of details by SQC

            { name: 'e', width: 120, sortable: false, resize: false, align: "center", search: false, hidden: (($("#RoleCode").val() == 22) || ($("#RoleCode").val() == 38) || ($("#RoleCode").val() == 54) || ($("#RoleCode").val() == 25) ? true : false) } // Finalize by SQC. Will be visible only at SQC Login
