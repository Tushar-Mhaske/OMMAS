...
function SaveRoadSafety1() {
    ...
    if ($('#frmRoadSafetyLayout1').valid()) {
        ...
        if (confirm("Do you want to save Details ?  RSA Stage & Inspection Date cannot be modified again once the details are saved. ")) {
...
function RedirectFinalizeRoad() {
    ...
    var id = EncryptedURL;
    //alert("ID  = " + id)
    if (confirm("Are you sure to finalize details ? Once Audit is finalized, chainage details cannot be added against this work")) {
        $.ajax({
