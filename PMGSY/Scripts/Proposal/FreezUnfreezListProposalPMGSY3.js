$(document).ready(function () {
    $("#divDisplayPhotographUploadView").hide();
    blockPage();
    ...
    if ($("#RoleID").val() == '2' || $("#RoleID").val() == '25' || $("#RoleID").val() == '36' || $("#RoleID").val() == '37' || $("#RoleID").val() == '55') {            //SRRDA or SRRDARCPLWE - listing Only 
        ...
        LoadSRRDAProposals($("#ddlState").val(), $("#ddlDistrict").val(), $("#ddlImsBatch").val(), $("#ddlImsStreams").val(), $("#ddlImsProposalTypes").val(), $("#ddlProposalStatus").val(), $("#ddlImsConnectivity").val());
    }
    else if ($("#RoleID").val() == '3') //STA
    {
        MordSRRDARoadProposalGrid(IMS_STATE, MAST_DISTRICT_ID)
    ...
    $('#btnAddProposal').click(function () {
        $("#accordion div").html("");
        $("#accordion h3").html(
                "<a href='#' style= 'font-size:.9em;' >Add " + $("#ddlImsProposalTypes option:selected").text() + " Proposal Details PMGSY3</a>" +
        ...
        $('#accordion').show('slow', function () {
            ...
            if ($("#ddlImsProposalTypes").val() == "P") {
                ...
                $("#divProposalForm").load('/Proposal/CreateProposalPMGSY3/' + $("#ddlImsYear").val() + "$" + $("#ddlMastBlockCode").val() + "$" + $("#ddlImsBatch").val() + "$" + $("#ddlImsStreams").val(), function () {
    ...
    $("#btnListProposal").click(function () {
      ...
      //  blockPage();
        ...
        LoadSRRDAProposals($("#ddlState").val(), $("#ddlDistrict").val(), $("#ddlMastBlockCode").val(), $("#ddlImsYear").val(), $("#ddlImsStreams").val(), $("#ddlImsProposalTypes").val(), $("#ddlProposalStatus").val(), $("#ddlImsConnectivity").val());
...
function ShowDetails(id) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divProposalForm").load('/Proposal/DetailsPMGSY3Proposal?id=' + id, function () {
            $.validator.unobtrusive.parse($('#divProposalForm'));
...
function EditDetails(id) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divProposalForm").load('/Proposal/EditProposalPMGSY3/' + id, function (response) {
            $.validator.unobtrusive.parse($('#divProposalForm'));
            unblockPage();
        });
        $('#divProposalForm').show('slow');
...
function LoadSRRDAProposals(STATE, MAST_DISTRICT_ID, MAST_BLOCK_CODE, YEAR, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT) {
    ...
    if (IMS_PROPOSAL_TYPE == "P") {
        ...
        SRRDARoadProposalGrid(STATE, MAST_DISTRICT_ID, MAST_BLOCK_CODE, YEAR, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT);
    }
    ...
    if (IMS_PROPOSAL_TYPE == "L") {
        ...
        SRRDALSBProposalGrid(STATE, MAST_DISTRICT_ID, MAST_BLOCK_CODE, YEAR, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT);
    }
...
function RoadProposalGrid(IMS_YEAR, MAST_BLOCK_ID, IMS_BATCH, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT) {
    ...
    jQuery("#tbProposalList").jqGrid({
        ...
        colNames: ['District', 'Block', "Package Number", "Year", "Road Name", "1000+", "999-500", "499-250", "Less Than 250", "Total Habitations", "Pavement Length (in Kms.)",
            "MoRD Share (in Lakhs)", $('#PMGSYScheme').val() == 2 ? "State Share Excluding higher Specification (in Lakhs)" : "State Share (in Lakhs)", "Higher Specification Cost (in Lakhs)",
            ...
            "Total State Share (in Lakhs)", "Total Share Cost (in Lakhs)", /*"Higher Specification Cost (in Lakhs)",*/ "Stage Construction", "STA (Scrutiny Date)", "PTA (Scrutiny Date)",
            "Status", "Habitations", "Traffic Intensity", "CBR Details", "Upload", "Technology Details", "View", "Edit", "Delete", "Proposal Status", "Forest Clearence"],
...
function MordSRRDARoadProposalGrid(IMS_STATE, MAST_DISTRICT_ID) {
    blockPage();
    jQuery("#tbSRRDAProposalList").jqGrid({
        url: '/Proposal/MordGetProposalsForSRRDAFreezeUnfreeze',
        datatype: "json",
        mtype: "POST",
        colNames: ['State', 'District', "Block", "Package ID", "Scheme", "Road Name", "Road Length", "Bridge Name", "Bridge Length", "Progress Status", "Freeze / Unfreeze"],
        colModel: [
                    { name: 'State', index: 'State', width: 100, sortable: false, align: "center" },
                    ...
                    { name: 'PackageNumber', index: 'PackageNumber', width: 100, sortable: false, align: "center" },
                     ...
                     { name: 'Scheme', index: 'Scheme', width: 100, sortable: false, align: "center" },
                      { name: 'RoadName', index: 'RoadName', width: 200, sortable: false, align: "left" },

                        { name: 'RoadLength', index: 'RoadLength', width: 100, sortable: false, align: "center" },

                        { name: 'BridgeName', index: 'BridgeName', width: 200, sortable: false, align: "left" },
                        { name: 'BridgeLength', index: 'BridgeLength', width: 100, sortable: false, align: "center" },
        ...
        postData: { "IMS_STATE": IMS_STATE, "MAST_DISTRICT_ID": MAST_DISTRICT_ID},
...
function SRRDARoadProposalGrid(STATE, MAST_DISTRICT_ID, MAST_BLOCK_CODE, YEAR, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT) {
    blockPage();
    jQuery("#tbSRRDAProposalList").jqGrid({
        url: '/Proposal/GetProposalsForSRRDAFreezeUnfreeze',
        datatype: "json",
        mtype: "POST",
        colNames: ['State', 'District', "Block", "Package ID", "Scheme","Year", "Road Name", "Road Length", "Expenditure","Bridge Name", "Bridge Length", "Progress Status","Freeze / Unfreeze"],
        colModel: [
                    ...
                    { name: 'PackageNumber', index: 'PackageNumber', width: 80, sortable: false, align: "center" },
                     ...
                     { name: 'Scheme', index: 'Scheme', width: 60, sortable: false, align: "center" },
                     { name: 'Year', index: 'Year', width: 100, sortable: false, align: "center" },

                      { name: 'RoadName', index: 'RoadName', width: 180, sortable: false, align: "left" },

                        { name: 'RoadLength', index: 'RoadLength', width: 100, sortable: false, align: "right" },
                         { name: 'Exp', index: 'Exp', width: 80, sortable: false, align: "right", hidden: true },//Added hiddden property on 13-10-2021

                        { name: 'BridgeName', index: 'BridgeName', width: 180, sortable: false, align: "left" },
                        { name: 'BridgeLength', index: 'BridgeLength', width: 70, sortable: false, align: "right" },
                         { name: 'Status', index: 'Status', width: 80, sortable: false, align: "center" },
                                      { name: 'Finalize', index: 'Finalize', height: 'auto', width: 80, align: "center", hidden: false },
        ],
        postData: { "STATE": STATE, "MAST_DISTRICT_ID": MAST_DISTRICT_ID, "MAST_BLOCK_CODE": MAST_BLOCK_CODE, "YEAR": YEAR, "IMS_STREAM": IMS_STREAM, "IMS_PROPOSAL_TYPE": IMS_PROPOSAL_TYPE, "IMS_PROPOSAL_STATUS": IMS_PROPOSAL_STATUS, "IMS_UPGRADE_CONNECT": IMS_UPGRADE_CONNECT },
        pager: jQuery('#dvSRRDAProposalListPager'),
        viewrecords: true,
        recordtext: '{2} records found',
        caption: "&nbsp;&nbsp;Road Proposals",
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    alert("Proposal Details Freezed Successfully.");
...
function SRRDALSBProposalGrid(IMS_YEAR, MAST_DISTRICT_ID, MAST_BLOCK_CODE, IMS_BATCH, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT) {
    jQuery("#tbSRRDALSBProposalList").jqGrid({
        ...
        colModel: [
                        ...
                        { name: 'TotalStateShare', index: 'TotalStateShare', width: 60, sortable: false, align: "right", },
                        { name: 'TotalShareCost', index: 'TotalShareCost', width: 60, sortable: false, align: "right", hidden: true },
                        { name: 'ShowDetails', index: 'ShowDetails', width: 50, sortable: false, align: "center" },
                        { name: 'UploadDetails', index: 'UploadDetails', width: 50, sortable: false, align: "center" },
        ],
...
//added by abhinav on 29-10-2019
function GetPDFUploadView(id) {
    var param = id.split(',')[0];
    var isFileUploaded = id.split(',')[1];
    CloseProposalDetails();
    if (isFileUploaded)
    {
        $.ajax({
            url: '/Proposal/PdfFileUploadView/' + param,
            type: "GET",
            cache: false,
            beforeSend: function () {
                blockPage();
            ...
            success: function (response) {
                ...
                $("#divDisplayPhotographUploadView").html('');
                $("#divDisplayPhotographUploadView").html(response);
                $("#accordionMonitorsInspection").show("slow");
    ...
    {
        if (confirm("Is forest clearence required ?\nif yes, click OK")) {

            if (confirm("Is clearence obtained ?\nif yes, click OK")) {
                $.ajax({
                    url: '/Proposal/PdfFileUploadView/' + param,
                    type: "GET",
                    cache: false,
