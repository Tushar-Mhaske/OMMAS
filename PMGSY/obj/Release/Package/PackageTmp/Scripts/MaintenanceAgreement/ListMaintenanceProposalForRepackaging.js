$(document).ready(function () {
    ...
    $("#ddlBlocks,#ddlYears,#ddlBatchs").change(function () {
        ...
        FillInCascadeDropdown({ userType: $(this).find(":selected").val() },
                   "#ddlPackages", "/MaintenanceAgreement/PopulateMaintenancePackagesForRepackaging?id=" + $('#ddlBlocks option:selected').val() + "$" + $('#ddlYears option:selected').val() + "$" + $('#ddlBatchs option:selected').val());
...
function LoadProposalList() {
    ...
    jQuery("#tblstProposalRepackage").jqGrid({
        url: '/MaintenanceAgreement/GetMaintenanceProposalListForRepackaging',
        datatype: "json",
        mtype: "POST",
        ...
        colNames: ['Block', 'Batch', 'Year','Old Package No.', 'New Package No.', 'Road Name / Bridge Name', 'Road Length (in Kms) / Bridge Length (in Mtrs.)', 'Change'],
        colModel: [
                        ...
                        { name: 'IMS_PACKAGE_ID_OLD', index: 'IMS_PACKAGE_ID_OLD', height: 'auto', width: 100, align: "center", search: false },
                        { name: 'IMS_PACKAGE_ID', index: 'IMS_PACKAGE_ID', height: 'auto', width: 100, align: "center", search: false },
        ...
        sortname: 'ROAD_NAME',
        sortorder: "asc",
        caption: "&nbsp;&nbsp; Maintenance Proposal List",
        hidegrid: true,
        height: 'auto',
...
function AddRepackagingDetails(IMS_PR_ROAD_CODE) {
    ...
    $('#accordion').show('fold', function () {
        ...
        $("#divAddPackage").load("/MaintenanceAgreement/AddMaintenanceRepackagingDetails?" + $.param({ ProposalCode: IMS_PR_ROAD_CODE }), function () {
