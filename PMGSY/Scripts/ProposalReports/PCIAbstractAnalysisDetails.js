...
function PCIAnalysisStateReportGrid(stateCode, stateName, districtCode, districtName, blockCode, flag, routeType) {
    ...
    $.blockUI({ message: '<img src="/Content/images/ajax-loader.gif"/>' });
    $("#tbPCIAbstractAnalysisReport").jqGrid('GridUnload');
    $("#tbPCIAbstractAnalysisReport").jqGrid({
        url: '/ProposalReports/PCIAbstractAnalysisListing',
        datatype: 'json',
        mtype: 'POST',
        ...
        pager: $("#dvPCIAbstractAnalysisReportPager"),
        footerrow: true,
        ...
        onSelectRow: function (id) {
             ...
             if ($("#hdnLevelId").val() == 6) //mord
            {
                 // flags = 'S';
                 flagstype = 'D';
                 GetPCIChart(params[0], params[1], roadWidth, districtCode, districtName, blockCode, flagstype, routeType, "dvPCIChart");
            ...
            else if ($("#hdnLevelId").val() == 4)//state
            {
                //flags = 'D';
                flagstype = 'B';
                GetPCIChart(stateCode, '', roadWidth, params[0], params[1], blockCode, flagstype, routeType, "dvPCIChart");
                $('#spanChartFor').html(params[1] + "   [" + roadWidth + "Kms.]");
    ...
    $("#tbPCIAbstractAnalysisReport").jqGrid('setGroupHeaders', {
...
function GetPCIChart(stateCode, stateName, rodeWidth, districtCode, districtName, blockCode, flag, routeType, ContainerDivID) {
    ...
    $.ajax({
        type: "POST",
        url: '/ProposalReports/PCIAbstractAnalysisChartListing/',
        data:
            {
                'StateCode': stateCode,
                'Flag': flag,
...
function CommonOptions(ContainerDivID) {
    ...
    switch (ContainerDivID) {
        case "dvPCIChart": colourArray = ['#DF0101', '#FE2E2E', '#FF9900', '#80E675', '#16BA04', '#86A033', '#614931', '#981A37']; break;
