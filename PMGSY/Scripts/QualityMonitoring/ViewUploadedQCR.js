...
function LoadExecutionGridCQC() {
    ...
    jQuery("#tbExecutionList").jqGrid({
        url: '/QualityMonitoring/GetExecutionList',
        datatype: "json",
        mtype: "POST",
        async: false,
        cache: false,
        postData: { yearCode: yearCode, districtCode: districtCode },
        colNames: ['District', 'Block', 'Sanctioned Year', 'Batch', 'Package No.', 'Road Code', 'Road Name', 'Road/LSB Cost (In Lacs)', 'Road Length(in Kms)/LSB Length(int Mtrs)', 'Maintenance Cost(in Lacs)', 'Proposal Type', 'View QCR Part-I PDF'/*, 'View PDF'*/],
        colModel: [
...
function ViewUploadedQCR(urlparameter) {
    ...
    $('#divAddQCR').load("/QualityMonitoring/AddQCR?idtemp=" + urlparameter, function () {
        ...
        $('#divListExecutionPDF').append(function () {
            ...
            jQuery("#tbExecutionListPDF").jqGrid({
                url: "/QualityMonitoring/GetQCRDetailsListToView?idtemp=" + urlparameter,
                datatype: "json",
                mtype: "POST",
                async: false,
                cache: false,
