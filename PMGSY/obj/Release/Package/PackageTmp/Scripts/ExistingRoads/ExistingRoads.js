...
function LoadExistingRoads(MAST_BLOCK_CODE, MAST_ROAD_CAT_CODE) {
    ...
    jQuery("#tbExistingRoadsList").jqGrid({
        ...
        colModel: [
                        { name: 'ERCode', index: 'ERCode', height: 'auto', width: 60, align: "left", sortable: true, search: true, editoptions: { maxlength: 3 } },
...
// Shift DRRP to new Block and District
                       ...
                       { name: 'ShiftDRRP', width: 40, sortable: false, resize: false, align: "center", sortable: false, search: false, hidden: ($("#StateCodeInSession").val() == 15 || $("#StateCodeInSession").val() == 37) ? false : true },
...
function ShiftDRRPToNewBlockAndDistrict(parameter) {

    $.blockUI({ message: '<img src="/Content/images/ajax-loader.gif"/>' });

    $('#dvShiftDRRPBlock').empty();
    $("#dvShiftDRRPBlock").load("/ExistingRoads/ShiftDRRPBlock?id=" + parameter, function () {

        $("#dvShiftDRRPBlock").dialog('open');
        $.unblockUI();
    })
...
function ShowDetails(id) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divExistingRoadsForm").load('/ExistingRoads/ViewExistingRoads/' + id, function () {
            $.validator.unobtrusive.parse($('#divExistingRoadsForm'));
            unblockPage();
        });
        $("#divExistingRoadsForm").show("slow");
...
function SearchExistingRoadList() {
    ...
    $('#tbExistingRoadsList').setGridParam({
        url: '/ExistingRoads/GetExistingRoadsList', datatype: 'json'
    });
