

jQuery.validator.addMethod("monthvalidator", function (value, element, param) {

    var IsMonthly = $('#rdbMonth').val();
    var month = $('#Month').val();

    if ($("#rdbMonth").is(":checked")) {
        if (IsMonthly == 2 && month == 0) {
            return false;
        }
        else {
            return true;
        }
    } else {
        return true;
    }
});

jQuery.validator.unobtrusive.adapters.addBool("monthvalidator");



$(document).ready(function () {

    
    //$("#rdbSrrda").attr('checked', true);
    $("#spanState").hide();
    $("#spanSrrda").show();
   // var month = $("#Month option:selected").val();
  //  var year = $("#Year option:selected").val();
   
    
    $("#btnView").click(function () {

        

        //if ($("#Month option:selected").val() == "0") {
        // // alert("Select Accounting month.");
        //    return false;
        //}
        //else 

        if ($("#rdbMonth").is(":checked")) {
            if (($("#Month option:selected").val() == "0")) {
                //alert("Select Accounting month.");
                return false;
            }
        }

        if ($("#Year option:selected").val() == "0") {
            //alert("Select Accounting Year");
            return false;
        }
        else {           

            if ($("#rdbSrrda").is(':checked')) {

                $(function () {

                    $("#tdlblDPIU").hide();
                });
            }
            else {
                $("#tdlblDPIU").show();
            }
            $("#lblAgency").text($("#Agency option:selected").text());

            $("#lblDPIU").text($("#Piu option:selected").text());
            
            if ($("#rdbMonth").is(":checked")) {
                $(function () {
                    $("#tdlblYear").hide();
                    $("#tdlblMonthYear").show();

                    $("#lblMonthYear").text(($("#Month option:selected").text()) + '-' + ($("#Year option:selected").text()));
                });
            }
            if ($("#rdbAnnual").is(":checked")) {
                $(function () {
                    $("#tdlblMonthYear").hide();
                    $("#tdlblYear").show();

                    $("#lblYear").text($("#Year option:selected").text());
                });
            }

            if ($("#rdbSrrda").is(":checked") && $("#rdbAnnual").is(":checked")) {
                //if ($("#Piu option:selected").val() == 0) {
                data = { month: 0, Year: $("#Year option:selected").val(), ndcode: $("#Agency option:selected").val(), rlevel: 1, allpiu: 0 };
                // }
                // else {
                //     data = { month: 0, Year: $("#Year option:selected").val(), ndcode: $("#Piu option:selected").val(), rlevel: 1, allpiu: 0 };
                // }
                ValidateForm(data);
            }

            if ($("#rdbSrrda").is(":checked") && $("#rdbMonth").is(":checked")) {
                // if ($("#Piu option:selected").val() == 0) {
                data = { month: $("#Month option:selected").val(), Year: $("#Year option:selected").val(), ndcode: $("#Agency option:selected").val(), rlevel: 1, allpiu: 0 };
                //  }
                // else {
                // data = { month: 0, Year: $("#Year option:selected").val(), ndcode: $("#Piu option:selected").val(), rlevel: 1, allpiu: 0 };
                // }
                ValidateForm(data);
            }

            if ($("#rdbPiu").is(":checked") && $("#rdbAnnual").is(":checked")) {
                if ($("#Piu option:selected").val() == 0) {
                    data = { month: 0, Year: $("#Year option:selected").val(), ndcode: $("#Agency option:selected").val(), rlevel: 2, allpiu: 1 };
                }
                else {
                    data = { month: 0, Year: $("#Year option:selected").val(), ndcode: $("#Piu option:selected").val(), rlevel: 2, allpiu: 0 };
                }
                ValidateForm(data);
            }

            if ($("#rdbPiu").is(":checked") && $("#rdbMonth").is(":checked")) {
                if ($("#Piu option:selected").val() == 0) {
                    data = { month: $("#Month option:selected").val(), Year: $("#Year option:selected").val(), ndcode: $("#Agency option:selected").val(), rlevel: 2, allpiu: 1 };
                }
                else {
                    data = { month: $("#Month option:selected").val(), Year: $("#Year option:selected").val(), ndcode: $("#Piu option:selected").val(), rlevel: 2, allpiu: 0 };
                }
                ValidateForm(data);
            }

            if ($("#LevelId").val() == 5 && $("#rdbAnnual").is(":checked")) {
                data = { month: 0, Year: $("#Year option:selected").val(), ndcode: $("#AdminNdCode").val(), rlevel: 2, allpiu: 0 };
                ValidateForm(data);
            }

            if ($("#LevelId").val() == 5 && $("#rdbMonth").is(":checked")) {
                data = { month: $("#Month option:selected").val(), Year: $("#Year option:selected").val(), ndcode: $("#AdminNdCode").val(), rlevel: 2, allpiu: 0 };
                ValidateForm(data);
            }
        }

       

        });
    
    $("#Month").change(function () {

        UpdateAccountSession($("#Month").val(), $("#Year").val());

    });

    $("#Year").change(function () {

        UpdateAccountSession($("#Month").val(), $("#Year").val());

    });

    
});

function ValidateForm(param) {

    $.ajax({
        url: "/Reports/ValidateParameter",
        type: "POST",
        data: param,
        success: function (data) {
            if (data.success == false) {
                $("#dvError").show();
                $("#dvError").html('<strong>Alert : </strong>' + data.message);

                return false;
            }
            else if (data.success == true) {
                loadFundTranGrid(param)
            }

        }
    });
}

function loadFundTranGrid(data) {
    $.blockUI({ message: '<img src="/Content/images/ajax-loader.gif"/>' });

    jQuery("#tbScheduleFundTransList").jqGrid("GridUnload");
    jQuery("#tbScheduleFundTransList").jqGrid({
        url: "/Reports/GetFundTransferredList",
        datatype: "json",
        mtype: "POST",
        colNames: ["Head of Account", "Current Month", "Previous Month"],
        colModel: [

                    { name: 'ITEM_HEADING', index: 'ITEM_HEADING', width: 400, sortable: false, align: "left" },
                    { name: 'CURRENT_AMT', index: 'CURRENT_AMT', width: 200, sortable: false, align: "right", editable: true, formatter: "number", formatoptions: { decimalPlaces: 2 } },
                    { name: 'PREVIOUS_AMT', index: 'PREVIOUS_AMT', width: 200, sortable: false, align: "right", editable: true, formatter: "number", formatoptions: {decimalPlaces:2}}
        ],
        rowNum: 1020,
        viewrecords: true,
        postData: data,
        caption: "&nbsp;&nbsp;Schedule of Fund Received",
        height: 'auto',
        //  maxheight: 100,
        shrinktofit: false,
        width: 'auto',
        autowidth: true,
        rownumbers: true,
        footerrow: true,
        userDataOnFooter: true,
        jsonReader: {
            repeatitems: false,
            total: "total",
            records: "records",
            page: "page",
            root: "rows",
            cell: "",
            id: "0"
        },
        loadComplete: function (jsondata) {

            //alert(jsondata.error.message);
            //return false;
            $("#tblHeading").show();


            $("#lblHeaderFormNumber").text(jsondata.reportHeader.formNumbr);
            $("#lblHeaderFundType").text(jsondata.reportHeader.fundType);
            $("#lblReportHeader").text(jsondata.reportHeader.heading);
            $("#lblReportReference").text(jsondata.reportHeader.referance);

            jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'ITEM_HEADING', "<b>" + jsondata.reportHeader.scheduleNo + "</b>");

            //jQuery("#tbScheduleCurrLibList").jqGrid('setCell', 'Total', 'ITEM_HEADING', "", { 'font-weight': 'bold', 'text-align': 'right' });

            jQuery("#tbScheduleFundTransList").jqGrid('setCell', 'Programme Fund ( MoRD)', 'ITEM_HEADING', "", { 'font-weight': 'bold' });

            jQuery("#tbScheduleFundTransList").jqGrid('setCell', 'Programme Fund (NABARD)', 'ITEM_HEADING', "", { 'font-weight': 'bold' });

            //jQuery("#tbScheduleFundTransList").jqGrid('setCell', 'Balance', 'ITEM_HEADING', "", { 'font-weight': 'bold' });
            //jQuery("#tbScheduleFundTransList").jqGrid('setCell', 'Balance', 'CURRENT_AMT', "", { 'font-weight': 'bold' });
            //jQuery("#tbScheduleFundTransList").jqGrid('setCell', 'Balance', 'PREVIOUS_AMT', "", { 'font-weight': 'bold' });



            //// jQuery("#tbScheduleCurrLibList").jqGrid('setCell', 'footerData', 'ITEM_HEADING', "", { 'font-weight': 'bold', 'text-align': 'right' });
            //// alert(jsondata.records);

            if (($("#rdbPiu").is(":checked") || $("#LevelId").val()==5) && $("#rdbMonth").is(":checked")) {

                jQuery("#tbScheduleFundTransList").jqGrid('footerData', 'set',
                    {
                        ITEM_HEADING: '<b>Balance as at the end of the month</b>',
                        CURRENT_AMT: jsondata.footerData.totCurrAmt % 1 == 0 ? jsondata.footerData.totCurrAmt + ".00" : jsondata.footerData.totCurrAmt,
                        PREVIOUS_AMT: jsondata.footerData.totPrevAmt % 1 == 0 ? jsondata.footerData.totPrevAmt + ".00" : jsondata.footerData.totPrevAmt
                    }

                );
            }
            else if (($("#rdbPiu").is(":checked") || $("#LevelId").val()==5) && $("#rdbAnnual").is(":checked")) {
                jQuery("#tbScheduleFundTransList").jqGrid('footerData', 'set',
                   {
                       ITEM_HEADING: '<b>Closing Balance</b>',
                       CURRENT_AMT: jsondata.footerData.totCurrAmt % 1 == 0 ? jsondata.footerData.totCurrAmt + ".00" : jsondata.footerData.totCurrAmt,
                       PREVIOUS_AMT: jsondata.footerData.totPrevAmt % 1 == 0 ? jsondata.footerData.totPrevAmt + ".00" : jsondata.footerData.totPrevAmt
                   }

               );
            }
            else {
                jQuery("#tbScheduleFundTransList").jqGrid('footerData', 'set',
                   {
                       ITEM_HEADING: '<b>Total</b>',
                       CURRENT_AMT: jsondata.footerData.totCurrAmt % 1 == 0 ? jsondata.footerData.totCurrAmt + ".00" : "Rs. " + jsondata.footerData.totCurrAmt,
                       PREVIOUS_AMT: jsondata.footerData.totPrevAmt % 1 == 0 ? jsondata.footerData.totPrevAmt + ".00" : jsondata.footerData.totPrevAmt
                   }

               );
            }

            //var allRowInGrid = jQuery("#tbScheduleCurrLibList").jqGrid("getDataIDs");
            //for (i = 1; i < 7; i++) {
            //    var rowId = allRowInGrid[i];
            //    var rowData = $('#tbScheduleCurrLibList').jqGrid('getRowData', rowId);
            //    if (rowData.CURRENT_AMT % 1 == 0) {
            //        rowData.CURRENT_AMT = rowData.CURRENT_AMT + "" + ".00";
            //        $('#tbScheduleCurrLibList').jqGrid('setRowData', rowId, rowData);
            //    }
            //    if (rowData.PREVIOUS_AMT % 1 == 0) {
            //        rowData.PREVIOUS_AMT = rowData.PREVIOUS_AMT + "" + ".00";
            //        $('#tbScheduleCurrLibList').jqGrid('setRowData', rowId, rowData);
            //    }
            //    else {
            //        $('#tbScheduleCurrLibList').jqGrid('setRowData', rowId, rowData);
            //    }


            //}
            //var grid = $("#tbScheduleCurrLibList"),
            //currMonSum = grid.jqGrid('getCol', 'CURRENT_AMT', false, 'sum');


            //prevMonSum = grid.jqGrid('getCol', 'PREVIOUS_AMT', false, 'sum');


            //if (currMonSum % 1 == 0) {
            //    currMonSum = currMonSum + "" + ".00";
            //}
            //if (prevMonSum % 1 == 0) {
            //    prevMonSum = prevMonSum + "" + ".00";
            //}

            //grid.jqGrid('footerData', 'set', { ID: 'curTotal:', CURRENT_AMT: currMonSum });

            //grid.jqGrid('footerData', 'set', { ID: 'prevTotal:', PREVIOUS_AMT: prevMonSum });
            // alert(jsondata.reportHeader.formNumbr);
            var month = $("#Month").val();
            var year = $("#Year").val();
            var yearPrev = "1999-2000";

            var isYear = $("#rdbAnnual").is(":checked");
            if (isYear) {
                if (year != 2000) {
                    year = year - 1;
                    yearPrev = $("#Year option[value=" + year + "]").text();
                }

                //modified by abhishek kamble 11-dec-2013
                //alert($("#Year option[value=" + year + "]").text());
                //jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'CURRENT_AMT', "Upto " + $("#Year option:selected").text());
                //jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'PREVIOUS_AMT', "Upto " + yearPrev);
                jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'CURRENT_AMT', "CURRENT YEAR : " + $("#Year option:selected").text());
                jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'PREVIOUS_AMT', "PREVIOUS YEAR : " + yearPrev);
            }
            else {

                if (month == 1) {
                    month = 12;
                    yearPrev = year - 1;
                }
                else {
                    yearPrev = year;
                    month = month - 1;
                }

                jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'CURRENT_AMT', "CURRENT MONTH AND YEAR : " + $("#Month option:selected").text() + " - " + year);
                jQuery("#tbScheduleFundTransList").jqGrid('setLabel', 'PREVIOUS_AMT', "PREVIOUS MONTH AND YEAR : " + $("#Month option[value=" + month + "]").text() + " - " + yearPrev);
            }

            //unblockPage();
            $.unblockUI();
        },
        loadError: function (xhr, status, error) {
           // unblockPage();
            alert(xhr.responseText);
            if (xhr.responseText == "session expired") {
                window.location.href = "/Login/SessionExpire";
            }
            else {
                window.location.href = "/Login/SessionExpire";
            }
            $.unblockUI();
        }

    }); //end of grid

}

function UpdateAccountSession(month, year) {
    $.ajax({
        url: "/Reports/UpdateAccountSession",
        type: "GET",
        async: false,
        cache: false,
        data:
            {
                "Month": month,
                "Year": year
            },
        success: function (data) {
            return false;
        },
        error: function () { }
    });
    return false;
}
