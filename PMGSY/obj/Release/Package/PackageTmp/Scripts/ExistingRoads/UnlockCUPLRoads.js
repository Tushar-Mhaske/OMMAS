...
function LoadProposalsForUpdate(IMS_YEAR, MAST_DISTRICT_ID, MAST_BLOCK_CODE, IMS_BATCH, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_UPGRADE_CONNECT) {
    ...
    jQuery("#tbUpdateProposalList").jqGrid({
        url: '/ExistingRoads/UnlockCUPLList',
        datatype: "json",
        mtype: "POST",
        colNames: ["Rank", "District Name", "Block Name", "Road Number ", "Candidate Road Length", "Candidate Road Length Eligibility (should be greater than equal to 5 km)", "Eligible CUCPL Length", "Road Name","Delete"],
        colModel: [
                    ...
                    { name: 'D', index: 'D', width: 150, sortable: false, align: "center" , hidden:true}
        ...
        caption: "&nbsp;&nbsp;CUPL Road List under PMGSY III",
        height: 'auto',
        width: 'auto',
        ...
        {
            $("#tbUpdateProposalList #dvUpdateProposalListPager1").css({ height: '31px' });

            $("#dvUpdateProposalListPager1_left").html("<input type='button' style='margin-left:27px' id='idFinalizeExistingRoad' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onClick = 'UnlockWholeBlockwiseCUPL();return false;' value='Delete CUPL'/>");
...
function UnlockWholeBlockwiseCUPL() {
   // var token = $('input[name=__RequestVerificationToken]').val();
  //  var EncryptedURL = $("#encryptedURLID").val();
   // var id = EncryptedURL;

    var BlockCode = $("#ddlMastBlockCode").val();
    var YearCode=$("#ddlImsYear").val();
    var BatchCode = $("#ddlImsBatch").val();

    var Block= $("#ddlMastBlockCode option:selected").text();
    var Year = $("#ddlImsYear option:selected").text();
    var Batch = $("#ddlImsBatch option:selected").text();



    if ($("#ddlMastBlockCode").val() == 0 || $("#ddlMastBlockCode").val() == -1)
    {
        alert("Please select Block.")
        return false;
    }
    else if ($("#ddlImsBatch").val() == -1 || $("#ddlImsBatch").val() == 0)
    {
        alert("Please select Batch.")
        return false;
    ...
    if (confirm("Are you sure to delete CUPL details for Block : " + Block+ ", Year : " + Year+", Batch : "+Batch)) {
        $.ajax({
            url: '/ExistingRoads/DeleteCUPLDetails/' + BlockCode + "$" + YearCode + "$" + BatchCode,
            type: "POST",
            cache: false,
            async: false,
            dataType: 'json',
          //  data: { "__RequestVerificationToken": token },
            ...
            //    $.unblockUI();
                alert("Your Request can not be processed right now. This may be due to Invalid Request, Session Timeout or Invalid Data Entry. Please verify and Try Again.");
                return false;
            }
        });
...
//        $.ajax({
//            type: 'POST',
//            url: '/ExistingRoads/DeleteCUPLDetails/' + urlparameter,
//            dataType: 'json',
