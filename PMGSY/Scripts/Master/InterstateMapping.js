...
$(document).ready(function () {
    ...
    $("#btnListProposal").click(function () {
        ...
        if ($("#ddlStates").val() == 0 || $("#ddlStates").val() == null) {
    ...
    //$("#ddlStates").change(function () {
    //   alert( $("#ddlStates").val())
    //   // $("#dvSearchParameter").load("");
    //    $("#dvSearchParameter").load("/Master/MasterQualityMonitorForMapping2?State=" + $("#ddlStates").val(), function () {
    //        $.validator.unobtrusive.parse($('#dvSearchParameter'));
    ...
    //    //            "#multiDist", "/Agreement/GetPackagesByYearandBlock?isPopulateFirstSelect=" + "false" + "&qmType=" + "S" + "&stateCode=" + $("#ddlStates").val());
...
function LoadProposalList() {
    ...
    var stateCodeDetails = $("#ddlStates").val();

    //alert("stateCodeDetails" + stateCodeDetails)
    //alert("DistListJoined" + DistListJoined)



    if (confirm("Are you sure to map selected Monitor and States Details ?")) {
        $.ajax({
            url: '/Master/MapInterState',
            type: "POST",
            cache: false,
            data: { "monitorIds": DistListJoined, "stateCode": stateCodeDetails, value: Math.random },
            beforeSend: function () {
                blockPage();
            },
...
function populateList() {
    ...
    jQuery("#tblstProposalRepackage").jqGrid({
        url: '/Master/FinalListAfterMapping',//MordGetEmargFinalList
        datatype: "json",
        mtype: "POST",
        postData: { value: Math.random },
        //  postData: { value: Math.random, monitorIds: DistListJoined, stateCode: stateCodeDetails },
        ...
        colNames: ['Receiving State', 'Monitor Name', 'Assigned Date', 'Assigned By', 'Status', 'Approved / Deactivated By', 'Approved / Deactivated Date', 'Action', 'Delete'],
        colModel: [

                        { name: 'ALLOWED_STATE_CODE', index: 'ALLOWED_STATE_CODE', height: 'auto', width: 100, align: "center", search: false },
                        { name: 'ADMIN_QM_CODE', index: 'ADMIN_QM_CODE', height: 'auto', width: 200, align: "center", search: false },
                        { name: 'ASSIGNED_DATE', index: 'ASSIGNED_DATE', height: 'auto', width: 100, align: "center", search: false },
                        { name: 'ASSIGNED_BY', index: 'ASSIGNED_BY', height: 'auto', width: 100, align: "center", search: false },




                        { name: 'APPROVED', index: 'APPROVED', height: 'auto', width: 100, align: "center", search: false }, // Status 


                        { name: 'APPROVED_BY', index: 'APPROVED_BY', height: 'auto', width: 100, align: "center", search: false }, //Approved / Deactivated By

                        { name: 'DATE', index: 'DATE', height: 'auto', width: 100, align: "center", search: false }, //Approved/ Deactivated Date

                        { name: 'a', index: 'a', height: 'auto', width: 120, align: "center", search: false, hidden: ($("#RoleCode").val() == 9 ? false : true) },
                        { name: 'Delete', index: 'Delete', height: 'auto', width: 100, align: "center", search: false }, //Approved/ Deactivated Date

                       // { name: 'APPROVED_DATE', index: 'APPROVED_DATE', height: 'auto', width: 100, align: "left", search: false },

                      //  { name: 'DEACTIVATED_DATE', index: 'DEACTIVATED_DATE', height: 'auto', width: 100, align: "center", search: false },
        ...
        rowNum: 5000,
        //  rowNum: 15,
        rowList: [25, 50, 75, 100, 125],
        // rowList: [15, 20, 25],
        viewrecords: true,
        recordtext: '{2} records found',
        sortname: 'ROAD_NAME',
        sortorder: "asc",
        caption: "&nbsp;&nbsp; Monitors and State Mapping",
        hidegrid: true,
        height: 'auto',
        ...
        loadComplete: function () {
            if (($("#hdnRoleCodeOnLayout").val() == 9)) //for CQC hide these columns
            {
                $('#tblstProposalRepackage').jqGrid('hideCol', 'Delete');
            }
        },
        loadError: function () { }
    });
...
function ApproveInsterState(urlparameter) {

    if (confirm("Are you sure you want to Approve Details?")) {
        $.ajax({
            type: 'POST',
            url: '/Master/ApproveInterState?id=' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
...
//DeactivateInsterState
function DeactivateInsterState(urlparameter) {

    if (confirm("Are you sure you want to DeApprove Details?")) {
        $.ajax({
            type: 'POST',
            url: '/Master/DeApproveInterState?id=' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
            success: function (data) {
                if (data.success == true) {
                    alert("Details deapproved successfully");
                    populateList();
                    //$("#tbFinancialList").trigger('reloadGrid');
                    //$("#divAddFinancialProgress").html('');
                }
                else if (data.success == false) {
                    alert("Details not be deapproved.");
                }
            },
            error: function (xhr, ajaxOptions, thrownError) {
                //alert(xhr.responseText);
...
function DeleteMapInterStateDetails(urlparameter) {
    if (confirm("Are you sure you want to Delete Details?")) {
        $.ajax({
            type: 'POST',
            url: '/Master/DeleteMapInterStateDetails?id=' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
            success: function (data) {
                if (data.success == true) {
                    ...
                    alert("Details could not be deleted.");
                }
            },
            error: function (xhr, ajaxOptions, thrownError) {
                //alert(xhr.responseText);
