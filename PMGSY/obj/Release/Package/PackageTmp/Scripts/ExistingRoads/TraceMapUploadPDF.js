...
function ListPDFFiles(BlockCode) {
    ...
    jQuery("#tbPDFFilesList").jqGrid({
        url: '/ExistingRoads/ListInspMultiplePDFFiles',
        datatype: "json",
        mtype: "POST",
        colNames: ["Click Icon To View / Download PDF", "Delete", "Is Finalized", "Finalise"],
        colModel: [
...
function AnchorFormatter(cellvalue, options, rowObject) {
    var url = "/ExistingRoads/DownloadMultipleInspFile/" + cellvalue;
...
function DeletePDFFileDetails(id){
    if (confirm("Are you sure to delete the PDF and it's details ? ")) {

        $.ajax({
            url: "/ExistingRoads/DeleteMultipleInspFileDetails/",
            type: "POST",
            cache: false,
            data: {
