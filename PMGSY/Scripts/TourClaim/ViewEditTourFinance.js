function AddDistrictDetails(evt, scheduleCode, showOption) {
    ...
    LoadDistrictDetailsListFin1(scheduleCode);
}

function LoadDistrictDetailsListFin1(scheduleCode) {
    
    jQuery("#tbDistrictDetailsList").jqGrid('GridUnload');
    jQuery("#tbDistrictDetailsList").jqGrid({
        url: '/TourClaim/GetTourDistrictListFin1',
        datatype: "json",
        mtype: "POST",
        async: false,
        cache: false,
...
function LoadTravelClaimListFin1(scheduleCode) {
    ...
    jQuery("#tbTravelClaimList").jqGrid({
        ...
        colModel: [
            ...
            { name: 'START_DATE_OF_TRAVEL', index: 'END_DATE_OF_TRAVEL', height: 'auto', width: 80, align: "center", search: false },
            ...
            { name: 'AMOUNT_PASSED_FIN2', index: 'AMOUNT_PASSED_FIN2', height: 'auto', width: 50, align: "center", search: false },
            { name: 'REMARK_FIN2', index: 'REMARK_FIN2', height: 'auto', width: 80, align: "center", search: false },
...
function getTextBoxValueTravelEdit(SelectedId, scheduleCode) {
    ...
    if (txtAmt == "") {
        ...
        if (confirm("Do you want to update changes ?")) {
            $.ajax({
                url: "/TourClaim/AddSanctionAmtTravelFin1",
                type: "POST",
                async: false,
                cache: false,
...
function AddLodgeClaim(evt, scheduleCode, showOption) {
    ...
    LoadLodgeClaimListFin1(scheduleCode);
}

function LoadLodgeClaimListFin1(scheduleCode) {

    jQuery("#tbLodgeClaimList").jqGrid('GridUnload');
    jQuery("#tbLodgeClaimList").jqGrid({
        url: '/TourClaim/GetLodgeClaimListFin1',
        datatype: "json",
        mtype: "POST",
        async: false,
        cache: false,
        postData: { scheduleCode: scheduleCode },
        colNames: ['Lodge Claim Id', 'Tour Claim Id', 'Schedule Code', 'Date From', 'Date To', 'Type of Claim', 'Hotel Name', 'View Uploaded Bill', 'View Uploaded e-Receipt', 'Daily', 'Hotel', 'Amount Proposed Daily', 'Hotel Amount Proposed by CQC', 'Remark by CQC', 'Hotel Amount Proposed by Finance Approver', 'Remark by Finance Approver','Amount Proposed', 'Remark', 'Save Changes'],
        colModel: [
            ...
            { name: 'AMOUNT_PASSED_DAILY_CQC', index: 'AMOUNT_PASSED_DAILY_CQC', height: 'auto', width: 80, align: "center", search: false },
            { name: 'AMOUNT_PASSED_HOTEL_CQC', index: 'AMOUNT_PASSED_HOTEL_CQC', height: 'auto', width: 50, align: "center", search: false },
            { name: 'REMARK_CQC', index: 'REMARK_CQC', height: 'auto', width: 80, align: "center", search: false },
            { name: 'AMOUNT_PASSED_HOTEL_FIN2', index: 'AMOUNT_PASSED_HOTEL_FIN2', height: 'auto', width: 50, align: "center", search: false },
            { name: 'REMARK_FIN2', index: 'REMARK_FIN2', height: 'auto', width: 80, align: "center", search: false },
    ...
    jQuery("#tbLodgeClaimList").jqGrid('setGroupHeaders', {
        ...
        groupHeaders: [
            ...
            //{ startColumnName: 'AMOUNT_PASSED_DAILY_CQC', numberOfColumns: 2, titleText: '<center>Amount Proposed by CQC</center>' },
            //{ startColumnName: 'AMOUNT_PASSED_DAILY', numberOfColumns: 2, titleText: '<center>Amount Proposed</center>' },
        ]
    });
}
...
function getTextBoxValueLodgeEdit(SelectedId, scheduleCode) {
    ...
    if (txtAmt == undefined) {
        ...
        $.ajax({
            url: "/TourClaim/AddSanctionAmtLodgeFin1",
            type: "POST",
            async: false,
            cache: false,
...
function PerformInspectionOfRoad(evt, scheduleCode, showOption) {
    ...
    LoadInspectionHonorariumListFin1(scheduleCode);
}

function LoadInspectionHonorariumListFin1(scheduleCode) {

    jQuery("#tbInspectionOfRoadsHonorarium").jqGrid('GridUnload');
    jQuery("#tbInspectionOfRoadsHonorarium").jqGrid({
        url: '/TourClaim/GetInspectionHonorariumListFin1',
        datatype: "json",
        mtype: "POST",
        async: false,
        cache: false,
        postData: { scheduleCode: scheduleCode },
        colNames: ['HONORARIUM_INSPECTION_ID', 'TourClaimId', 'Schedule Code', 'Date of Inspection', 'Work Type', 'Amount Claimed', 'Amount Proposed by CQC', 'Remark by CQC', 'Amount Proposed by Finance Approver', 'Remark by Finance Approver', 'Amount Proposed', 'Remark', 'Save Changes'],
        colModel: [
...
function getTextBoxValueInspEdit(SelectedId, scheduleCode) {
    ...
    if (txtAmt == "") {
        ...
        if (confirm("Do you want to update changes ?")) {
            $.ajax({
                url: "/TourClaim/AddSanctionAmtInspFin1",
                type: "POST",
                async: false,
                cache: false,
...
function MeetingWithPIU(evt, scheduleCode, showOption) {
    ...
    LoadMeetingHonorariumListFin1(scheduleCode);
}

function LoadMeetingHonorariumListFin1(scheduleCode) {

    jQuery("#tbMeetingHonorarium").jqGrid('GridUnload');
    jQuery("#tbMeetingHonorarium").jqGrid({
        url: '/TourClaim/GetMeetingHonorariumListFin1',
        datatype: "json",
        mtype: "POST",
        async: false,
        cache: false,
        postData: { scheduleCode: scheduleCode },
        colNames: ['HONORARIUM_MEETING_ID', 'TourClaimId', 'Schedule Code', 'Date of Meeting', 'Place of Meeting', 'View Uploaded attendance sheet', 'View Uploaded meeting details', 'View Uploaded Geotagged Photograph', 'Amount Claimed', 'Amount Proposed by CQC', 'Remark by CQC', 'Amount Proposed by Finance Approver', 'Remark by Finance Approver', 'Amount Proposed', 'Remark', 'Save Changes'],
        colModel: [
...
function getTextBoxValueMeetingEdit(SelectedId, scheduleCode) {
    ...
    if (txtAmt == "") {
        ...
        if (confirm("Do you want to update changes ?")) {
            $.ajax({
                url: "/TourClaim/AddSanctionAmtMeetingFin1",
                type: "POST",
                async: false,
                cache: false,
...
function AddMiscellaneousClaim(evt, scheduleCode, showOption) {
    ...
    LoadMiscellaneousClaimListFin1(scheduleCode);
}

function LoadMiscellaneousClaimListFin1(scheduleCode) {

    jQuery("#tbMiscellaneousClaimList").jqGrid('GridUnload');
    jQuery("#tbMiscellaneousClaimList").jqGrid({
        url: '/TourClaim/LoadMiscellaneousClaimListFin1',
        datatype: "json",
        mtype: "POST",
        async: false,
        cache: false,
        postData: { scheduleCode: scheduleCode },
        colNames: ['Mis Id', 'Tour Claim Id', 'Schedule Code', 'Date', 'Description', 'View Uploaded File', 'Amount Claimed', 'Amount Proposed by CQC', 'Remark by CQC', 'Amount Proposed by Finance Approver', 'Remark by Finance Approver', 'Amount Proposed', 'Remark', 'Save Changes'],
        colModel: [
...
function getTextBoxValueMiscellaneousEdit(SelectedId, scheduleCode) {
    ...
    if (txtAmt == "") {
        ...
        if (confirm("Do you want to update changes ?")) {
            $.ajax({
                url: "/TourClaim/AddSanctionAmtMiscellaneousFin1",
                type: "POST",
                async: false,
                cache: false,
...
function ForwardToFin2(id) {

    if ($('#officeAssistantFinance').val() == "")
        alert("Please enter Office Assistant Finance name")
    else if (confirm("Do you want to forward to approver ?")) {
        $.ajax({
            url: "/TourClaim/ForwardToFin2",
            type: "POST",
            async: false,
            cache: false,
            data: { id: id },
...
function OpenNqmLetter() {
    var scheduleCode = $('#scheduleCodeFin1').val();
...
function ViewPreviewDetailsFin1(scheduleCode) {

    $('.tab').hide('slow');
    $('#showMainForm').hide('slow');

    $('#previewDiv').show('slow', function () {
        blockPage();
        $("#divViewPreviewDetails").load('/TourClaim/ViewSummaryDetailsReportFin1/?scheduleCode=' + scheduleCode, function () {
            unblockPage();
        });
    });
