        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    if (levelId == 4) {
                        $.ajax({
                            url: '/TEO/AddAutoEntryTOB/' + urlParam,
                            type: "POST",
                            async: false,
                            cache: false,
                            //data:
...
function LoadGrid(month, year, fromDate, toDate, transType, mode) {
    ...
    jQuery("#tblTEOList").jqGrid({
        url: '/TEO/GetTEOListforTransferOfBalances',
        datatype: "json",
        mtype: "POST",
        colNames: ['TEO Number', 'TEO Date', 'Transaction Name', 'Gross Amount', 'Details Amount', 'Status', 'Edit', 'Delete', 'Status'],
        colModel: [
