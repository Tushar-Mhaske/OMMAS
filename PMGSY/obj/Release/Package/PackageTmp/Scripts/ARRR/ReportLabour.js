$(document).ready(function () {
    ...
    $('#StateList_PackageWiseDetails').change(function () {
     //   alert("T")
        $("#Year_PackageWiseDetails").empty();
        $.ajax({
            url: '/ARRR/PopulateFinancialYearsByStateForARRRLabour',
            type: 'POST',
            beforeSend: function () {
                $.blockUI({ message: '<img src="/Content/images/ajax-loader.gif" />' });
            },
            data: { stateCode: $("#StateList_PackageWiseDetails").val() },

         //   data: AddAntiForgeryToken({ stateCode: $("#StateList_PackageWiseDetails").val() }),
            success: function (jsonData) {
                for (var i = 0; i < jsonData.length; i++) {
                    if (jsonData[i].Selected == true) {
                        $("#Year_PackageWiseDetails").append("<option value='" + jsonData[i].Value + "'>" + jsonData[i].Text + "</option>");
                    }
                    else {
                        $("#Year_PackageWiseDetails").append("<option value='" + jsonData[i].Value + "'>" + jsonData[i].Text + "</option>");
                    }
...
function loadDistrict(statCode) {
    ...
    if (statCode > 0) {
        if ($("#Year_PackageWiseDetails").length > 0) {
            $.ajax({
                url: '/ARRR/PopulateYearsForARRRDetails',
                type: 'POST',
                ...
                success: function (jsonData) {
                    ...
                    if ($("#PhaseYear").val() > 0) {
                        $("#Year_PackageWiseDetails").val($("#PhaseYear").val());
                    
                        $("#Year_PackageWiseDetails").trigger('change');
                    }
