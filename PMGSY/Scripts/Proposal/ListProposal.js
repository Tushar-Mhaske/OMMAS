...
function STAListRoadProposals(IMS_YEAR, MAST_DISTRICT_ID, IMS_BATCH, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS) {
    ...
    if ($("#ddlState").val() > 0) {
        ...
        jQuery("#tbStaProposalList").jqGrid({
            ...
            rownumbers: true,
            loadComplete: function () {
                if ($("#PMGSYScheme").val() == 3 || $("#PMGSYScheme").val() == 4) {
...
function MordListRoadProposals(IMS_YEAR, MAST_STATE_ID, MAST_DISTRICT_ID, IMS_BATCH, IMS_STREAM, IMS_PROPOSAL_TYPE, IMS_PROPOSAL_STATUS, IMS_UPGRADE_CONNECT, IMS_AGENCY) {
    ...
    jQuery("#tbMORDProposalList").jqGrid({
        ...
        colModel: [
                    ...
                    { name: 'a', width: 50, sortable: false, resize: false, align: "center", search: false }// Delete Non Sanctioned and Non Dropped Proposals
        ],
...
function DeleteNonSanctionedAndNonDroppedProposalDetails(urlparameter) {


    if (confirm("Are you sure you want to delete Proposal details?")) {
        $.ajax({
            type: 'POST',
            url: '/Proposal/DeleteNonSanctionedAndNonDroppedProposal/' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
            success: function (data) {
                ...
                {
                    alert("Proposal details deleted successfully");
                ...
                {
                    alert("Proposal details can not be deleted.");
                }
            },
