...
function ListQualityFiles(IMS_PR_ROAD_CODE) {
    ...
    var FinalizedByAuditor = $("#IsFinalized").val();

    jQuery("#tbFilesList").jqGrid('GridUnload');

    jQuery("#tbFilesList").jqGrid({
        url: '/Execution/ListPDFByAuditor?' + Math.random(),
        datatype: "json",
        mtype: "POST",
        colNames: ["File",'Description of File',"Download", "Delete"],
        colModel: [
                   ...
                   { name: 'EXEC_FILE_NAME', index: 'EXEC_FILE_NAME', width: 150, sortable: false, align: 'center', editable: false },

                   { name: 'download', index: 'download', width: 80, sortable: false, align: 'center', editable: false, hidden: true, title: false },
                   { name: 'Delete', index: 'Delete', width: 80, sortable: false, align: "center", editable: false, hidden: isHiddenByAuditor(FinalizedByAuditor), title: false }
        ...
        caption: "PDF File Details uploaded By Auditor",
        height: '100',
        sortname: 'Image',
        rownumbers: true,
...
function isHiddenByAuditor(FinalizedByAuditor) {

    if (FinalizedByAuditor == 'Y')
    {
        return true;
    }
    else
...
function DownLoadImage(cellvalue) {
    ...
    var url = "/Execution/DownloadPDFUploadedByAuditor/" + cellvalue;
...
function DeleteFileDetails(cellvalue) {
    ...
    if (confirm("Are you sure to delete File ? ")) {
        ...
        $.ajax({
            url: "/Execution/DeletePDFDetailsByAuditor/" + cellvalue,
            type: "POST",
            cache: false,
            //data: {
