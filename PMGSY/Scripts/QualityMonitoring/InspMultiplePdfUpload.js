...
function ListPDFFiles(QM_OBSERVATION_ID) {
    ...
    jQuery("#tbPDFFilesList").jqGrid({
        url: '/QualityMonitoring/ListInspMultiplePDFFiles',
        datatype: "json",
        mtype: "POST",
        colNames: ["PDF", "Description", "Uploaded By", "Edit", "Delete", "Action" , "Is Finalised" , "Finalise"],
        colModel: [
...
function AnchorFormatter(cellvalue, options, rowObject) {
    var url = "/QualityMonitoring/DownloadMultipleInspFile/" + cellvalue;
...
function DeletePDFFileDetails(QM_OBSERVATION_ID, FILE_NAME) {
    ...
    if (confirm("Are you sure to delete the PDF and it's details ? ")) {
        ...
        $.ajax({
            url: "/QualityMonitoring/DeleteMultipleInspFileDetails/",
            type: "POST",
            cache: false,
            data: {
...
function FinalisePDFDetails(fileID) {

    if (confirm("Are you sure to Finalise the details ? ")) {

        $.ajax({
            url: "/QualityMonitoring/FinaliseMultipleInspFile/",
            type: "POST",
            cache: false,
            data: {
                QM_FILE_ID: fileID , value: Math.random()
            },
            beforeSend: function () {
                blockPage();
            },
