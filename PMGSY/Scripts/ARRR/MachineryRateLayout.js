$(document).ready(function () {
    $.validator.unobtrusive.parse('#frmMachineryRate');

    $("#idFilterDivMch").click(function () {
        $("#idFilterDivMch").removeClass("ui-icon-circle-triangle-s").addClass("ui-icon-circle-triangle-n");
        $("#frmMachineryRate").toggle("slow");
    });

    $('#ddlCategory').change(function () {
        $("#ddlItem").empty();
        $.ajax({
            url: '/ARRR/MachineryRateDetails/',
            type: 'POST',
    ...
    //    if ($('#frmMachineryRate').valid()) {
    //        $('#User_Action').val('A');
    //        $.ajax({
    //            url: '/ARRR/AddEditMachineryRateDetails/',
    //            async: false,
    //            type: 'POST',
    //            //data: form_data,
    //            data: $("#frmMachineryRate").serialize(),
    //            //contentType: false,
    //            //processData: false,
    ...
    $("#btnSave").click(function () {
        ...
        debugger;
        var allData = [];
        var ids = jQuery("#tblLoadMachineryFormList").jqGrid('getDataIDs');
        for (var i = 0; i < ids.length; i++) {
            ...
            if (!!txt) {
                if (txt > 0) {
                    ...
                    var rowData = jQuery('#tblLoadMachineryFormList').jqGrid('getRowData', rowId);
                    allData.push({
                        File: file,
                        ...
                        Form: $('#frmMachineryRate').serializeArray(),
        ...
        if ($("#ddlYear option:selected").val() > 0/*$('#frmLabourRate').valid()*/) {
            $('#User_Action').val('A');
            $.ajax({
                url: '/ARRR/AddMultipleMachineryData/',
                async: false,
                type: 'POST',
    ...
    LoadMachineryFormGrid();
});

function LoadMachineryFormGrid() {
    $("#tblLoadMachineryFormList").jqGrid('GridUnload');
    jQuery("#tblLoadMachineryFormList").jqGrid({
        url: '/ARRR/GetMachineryFormList',
        datatype: "json",
        mtype: "POST",
        colNames: ['Item Code', 'Category', 'Machinery Type', 'Output Unit', 'Output', 'Usage Unit', 'Upload File', 'Rate(Rs.)'],
        colModel: [
                    ...
                    { name: 'Output', index: 'Output', height: 'auto', width: 100, align: "left", sortable: true },
                     { name: 'Usage Unit', index: 'Usage Unit', height: 'auto', width: 130, align: "left", sortable: true },
        ...
        pager: jQuery('#dvLoadMachineryFormListpager'),
        rowNum: 10,
