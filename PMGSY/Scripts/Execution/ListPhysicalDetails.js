...
function EditRoadPhysicalProgress(urlparameter) {
    ...
    $("#divAddRoadProgress").load("/Execution/EditPhysicalRoadProgress/" + urlparameter, function (data) {
...
function AddPhysicalRoadProgress(IMS_ROAD_CODE) {
    ...
    if (isValid == true) {
        $("#divAddRoadProgress").load("/Execution/AddPhysicalRoadProgress/" + IMS_ROAD_CODE, function (data) {
            if (data.success == false) {
                alert(data.message);
            }
            else {
