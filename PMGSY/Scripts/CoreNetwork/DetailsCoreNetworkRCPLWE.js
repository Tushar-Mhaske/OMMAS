$(document).ready(function () {
    ...
    $("#btnFinalize").click(function () {
        ///Habitation Finalization is not mandatory for RCPLWE Road Finalization   change by SAMMED A. PATIL
        /*$.ajax({
            ...
            success: function (response) {
                ...
                if (response.Success) {*/
        blockPage();
        if (confirm("Once RCPLWE Road is Finalized, it can not be Edited and Deleted.\nAre you sure to Finalize it ?")) {
            $.ajax({
                url: '/CoreNetwork/FinalizeCoreNetworkRCPLWE',
                type: "POST",
                cache: false,
                beforeSend: function () {
                    blockPage();
                ...
                success: function (response) {
                    unblockPage();
                    if (response.Success) {
                        alert("RCPLWE Road Finalized successfully.");
...
function LoadHabitationGrid() {
    ...
    jQuery("#habitationDetailsCategory").jqGrid({
        ...
        colModel: [
                            ...
                            { name: 'PLAN_VILLAGE_NAME', index: 'PLAN_VILLAGE_NAME', width: 300, sortable: false, align: "center", hidden: true },
                            { name: 'PLAN_RD_NAME', index: 'PLAN_RD_NAME', width: 300, sortable: false, align: "center", hidden: true },
...
function LoadProposalList(PLAN_CN_ROAD_CODE) {
    ...
    jQuery("#tbProposalList").jqGrid({
        ...
        datatype: "json",
        mtype: "POST",
        colNames: ["Proposal Type", "Road / Bridge Name", "Year", "Batch", "Package", "New/Upgrade", "Road / Bridge Cost", "Road / Bridge Status"],
        colModel: [
