$(document).ready(function () {
 ...
 //    LoadExecutionGrid(nameRole);
    ...
    // Added Now Get Disrtict By State
    //$("#ddlMastStateCode").change(function () {

    //    FillInCascadeDropdown({ userType: $("#ddlMastStateCode").find(":selected").val() },
    //                "#ddlMastBlockCode", "/Execution/GetDistrictByState?districtCode=" + $('#ddlMastDistrictCode option:selected').val());
    //});
...
function LoadExecutionGrid(nameRole) {
    ...
    jQuery("#tbExecutionList").jqGrid({
        url: '/Execution/GetRoadSafetyATRProgressList',
        datatype: "json",
        mtype: "POST",
...
function isHiddenSQC(roleName) {
    ...
    else if (roleName == 'RSAAuditorDistrict') {
        return true;
    }
