...
$(document).ready(function () {
    ...
    $("#spnFundTrans").click(function () {
        $("#spnFundTrans").toggleClass("ui-icon ui-icon-circle-triangle-n").toggleClass("ui-icon ui-icon-circle-triangle-s");
        $("#dvFundTransfer").slideToggle("slow");
    });

    $("#btnViewDetails").click(function () {

        $("#dvFundTransfer").hide("slow");
        ...
        if ($("#frmAbstractFund").valid()) {
            $("#ddlSRRDA").attr("disabled", false);
            $.ajax({
                url: "/AccountReports/Account/AbstractFundReport/",
                type: "POST",
                async: false,
                cache: false,
                ...
                success: function (data) {
                    if (data.success == false) {
                        ...
                        $("#AbstractFundDetails1").html(data);
                        if ($("#LevelId").val() == 4) {
                            $("#ddlSRRDA").attr("disabled", true);
                        }
                    }
