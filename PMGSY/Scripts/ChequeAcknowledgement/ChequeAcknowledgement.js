...
$(document).ready(function () {
    ...
    $("#btnViewDetails").click(function () {
        ...
        accType = $("#ACCOUNT_TYPE option:selected").val();//Added by Srishti on 03-03-2023
                ...
                success: function (data) {
                    ...
                    if (data.success == true) {
                        ...
                        $("#ackVoucherDiv").load("/ChequeAcknowledgement/GetVoucherAckForm/", function () {
                            ...
                            loadGrid($("#BILL_MONTH").val(), $("#BILL_YEAR").val(), $("#DPIU").val(), accType); // added by Srishti 
    ...
    $("#btnViewVouchers").click(function () {
        ...
        var accType = $("#ACCOUNT_TYPE option:selected").val();
        var typeOfAccType = typeof $("#ACCOUNT_TYPE option:selected").val();
...
function SubmitChequeAckDetails(btnClick) {
    ...
    var typeOfAccType = typeof $("#ACCOUNT_TYPE option:selected").val();
    if (typeOfAccType != 'undefined')
        $("#ACC_TYPE").val($('#ACCOUNT_TYPE option:selected').val());
    else
        $("#ACC_TYPE").val("1");
        ...
        success: function (data) {
            ...
            if (data.Success) {
                ...
                if (finaliseFalg == "Y") {
                    ...
                    $("#tblForm").show();
                    loadGrid($("#BILL_MONTH").val(), $("#BILL_YEAR").val(), $("#DPIU").val(), $('#ACCOUNT_TYPE').val());
                }
...
function loadGrid(month, year, dpiu, accType) {
    ...
    $grid.jqGrid({
        ...
        url: '/ChequeAcknowledgement/ListChequeForAcknowledgment/' + month + "$" + year + "$" + dpiu + "$" + accType,
        datatype: 'json',
        mtype: 'POST',
        height: 'auto',
        rowNum: -1,
        postData: { 'months': $('#BILL_MONTH').val(), 'year': $('#BILL_YEAR').val(), 'DPIU': $('#DPIU').val(), 'ACC_TYPE': accType, 'AckUnackFlag': $("#AckUnackFlag").val() },
        rownumbers: true,
        autowidth: true,
        pginput: false,
        multiselect: true,
        pgbuttons: false,
        colNames: ['Cheque / Advice Issued Date', 'Cheque / Advice Date', 'Transaction Type', 'Account Type', 'Cheque / Advice Number', 'Payee Name', 'Cheque Amount (In Rs.)', 'Acknowledge', "", ""],
        colModel: [
            {
