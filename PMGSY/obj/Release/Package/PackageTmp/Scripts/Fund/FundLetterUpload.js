$(function () {
    ...
    $("#fileupload").bind("fileuploaddone", function (e, data) {
        ...
        $("#tbListFiles").trigger('reloadGrid');
...
$(document).ready(function ()
{
    ...
    ListFiles(stateCode,adminCode,fundType,fundingAgency,yearCode,transactionCode);
...
function ListFiles(stateCode, adminCode, fundType, fundingAgency, yearCode, transactionCode)
{

    blockPage();
    jQuery("#tbListFiles").jqGrid({
        url: '/Fund/ListFiles',
        datatype: "json",
        mtype: "POST",
        ...
        postData: { "STATE_CODE": stateCode,"ADMIN_CODE":adminCode,"FUND_TYPE":fundType,"FUNDING_AGENCY":fundingAgency,"YEAR":yearCode,"TRANSACTION":transactionCode},
        pager: jQuery('#dvPagerFiles'),
        rowNum: 5,
        viewrecords: true,
...
function DeleteFundAllocationFile(PLAN_FILE_NAME) {

    $("#divSuccess").hide("slow");
    if (confirm("Are you sure to delete the File ? ")) {
        $.ajax({
            url: "/Fund/DeleteFundAllocationFile/"+PLAN_FILE_NAME,
            type: "POST",
            cache: false,
            beforeSend: function () {
                blockPage();
            ...
            success: function (response) {
                unblockPage();
                var divData = $("#divAddFundAllocationForm").html();
                if (response.Success) {
                    $("#tbListFiles").trigger('reloadGrid');
                    $("#NumberofFiles").val(parseInt($("#NumberofFiles").val()) - 1);
                    $("#divAddFundAllocationForm").load('/Fund/FileUpload/' + $("#UrlParameter").val(), function (e) {
