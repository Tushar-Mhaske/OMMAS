...
function LoadCoreNetwork(RoadCode) {
    jQuery("#tbCoreNetwork").jqGrid({
        url: '/ExistingRoads/ListCoreNetworkByDRRPITNO',
        datatype: "json",
        mtype: "POST",
        postData: { MAST_ER_ROAD_CODE: RoadCode },
        colNames: ['System ID', 'Road No.', 'Road Name', 'Road From', 'Road To', 'Start Chainage [In Km]', 'End Chainage [In Km]', 'Length [In Km]', "Delete"],
        colModel: [
                            { name: 'PLAN_CN_ROAD_CODE', index: 'PLAN_CN_ROAD_CODE', height: 'auto', width: 80, align: "left", search: false },
                            { name: 'PLAN_CN_ROAD_NUMBER', index: 'PLAN_CN_ROAD_NUMBER', height: 'auto', width: 80, align: "left", search: false },
                            { name: 'PLAN_RD_NAME', index: 'PLAN_RD_NAME', height: 'auto', width: 240, align: "left", search: true },
                            { name: 'PLAN_RD_FROM', index: 'PLAN_RD_FROM', height: 'auto', width: 170, align: "left", search: false },
                            { name: 'PLAN_RD_TO', index: 'PLAN_RD_TO', height: 'auto', width: 170, align: "left", search: false },
                            ...
                            { name: 'DeleteRoad', index: 'DeleteRoad', height: 'auto', width: 90, align: "center", search: false },
        ],
        ...
        loadComplete: function () {
            ...
            $('#tbCoreNetwork').jqGrid('setGridWidth', '900');
        },
        loadError: function (xhr, ststus, error) {
...
function DeleteDetails(id) {
    if (confirm("Are you sure to delete the details ?")) {
        $.ajax({
            url: '/ExistingRoads/DeleteExistingCoreNetworkRoad/' + id,
            type: 'POST',
            dataType: 'Json',
            catche: false,
            async: false,
            ...
            success: function (response) {
                unblockPage();
                if (response.success) {
                    jQuery("#tbCoreNetwork").trigger("reloadGrid");
