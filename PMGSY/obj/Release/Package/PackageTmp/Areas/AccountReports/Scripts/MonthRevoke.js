...
$(document).ready(function () {
    ...
    $("#spnCurrentAssets").click(function () {
        $("#spnCurrentAssets").toggleClass("ui-icon ui-icon-circle-triangle-n").toggleClass("ui-icon ui-icon-circle-triangle-s");
    ...
    $("#Agency").change(function () {
        ...
        $.getJSON("/AccountReports/Account/GetAllDPIUOfSRRDA/", { ndcode: ndcode }, function (responseData) {
            var strOption = "";
            $("#Piu").empty();
...
function ShowReport(levelId) {
   ...
   // var StateSrrdaDpiu;
    ...
    if (Duration == 1)
    {
        ...
        {
            var data = { month: month, year: year, ndcode: ndcode, rlevel: reportLevel, allpiu: allpiu, duration: Duration, ToMonth: ToMonth, ToYear: ToYear };
            ValidateForm(data);
        }
    }
...
function loadCurrentAssetReport(data) {
    $.blockUI({ message: '<img src="/Content/images/ajax-loader.gif"/>' });

    $.ajax({
        url: '/AccountReports/Account/MonthRevokeReport/',
        type: 'POST',
        catche: false,
        async: false,
        data: data,
