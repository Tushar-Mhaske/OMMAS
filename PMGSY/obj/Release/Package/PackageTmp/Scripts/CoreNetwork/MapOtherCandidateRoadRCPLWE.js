...
$(document).ready(function () {
    ...
    $("#btnSaveCandidateRoad").click(function () {
        ...
        if ($("#frmMapDRRPRCPLWE").valid()) {

            blockPage();

            $.ajax({
                type: 'POST',
                url: '/CoreNetwork/MapCandidateRoad/',
                async: false,
                data: $("#frmMapDRRPRCPLWE").serialize(),
                beforeSend: function () {
                    blockPage();
                },
    ...
    $("#finalize").click(function () {
        ...
        if (confirm("Once Finalized,Another DRRP Roads will not be able to map.\n Are you sure you want to finalize DRRP Details?")) {
            $.ajax({
                type: 'POST',
                url: '/CoreNetWork/FinalizeMappedDRRPDetails?id=' + $("#CNCode").val(),
                dataType: 'json',
                async: false,
                cache: false,
                success: function (data) {
    ...
    $("#txtEndChainage").blur(function () {
        ...
        $("#txtLengthOfRoad").val(parseFloat(roadLength).toFixed(2));
...
function DeleteCandidateRoad(urlparameter) {
    if (confirm("Are you sure you want to delete DRRP Details?")) {
        $.ajax({
            type: 'POST',
            url: '/CoreNetWork/DeleteMappedDRRPDetails/' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
            success: function (data) {
...
function EditCandidateRoad(urlparameter) {
    ...
    $("#dvMapDRRPRCPLWE").load('/CoreNetWork/EditMappedDRRPDetails/' + urlparameter, function () {
        $.validator.unobtrusive.parse($('#dvMapDRRPRCPLWE'));
    });
}