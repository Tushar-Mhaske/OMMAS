...
$(document).ready(function () {
    ...
    $("#ddlRoadCategories").change(function () {
        ...
        if ($("#ddlBlock option:selected").val() == 0) {
            ...
            FillInCascadeDropdown({ userType: $("#ddlRoadCategories").find(":selected").val() },
                "#ddlDRRP", "/CoreNetwork/GetRoadNameByRoadCodeVVP?roadName=" + $('#ddlRoadCategories option:selected').val() + "&blockName=" + $("#ddlBlock option:selected").val());
        }
    ...
    $("#btnSaveCandidateRoad").click(function () {
        ...
        if ($("#frmMapDRRP").valid()) {
            ...
            $.ajax({
                type: 'POST',
                url: '/CoreNetwork/MapCandidateRoadVVP/',
                async: false,
    ...
    $("#finalize").click(function () {
        ...
        if (confirm("Once Finalized,Another DRRP Roads will not be able to map.\n Are you sure you want to finalize DRRP Details?")) {
            $.ajax({
                type: 'POST',
                url: '/CoreNetWork/FinalizeMappedDRRPDetailsVVP?id=' + $("#CNCode").val(),
                dataType: 'json',
                async: false,
                cache: false,
                success: function (data) {
    ...
    $("#definalize").click(function () {
        ...
        if (confirm("Are you sure you want to definalize DRRP Details?")) {
            $.ajax({
                type: 'POST',
                url: '/CoreNetWork/DeFinalizeMappedDRRPDetails?id=' + $("#CNCode").val(),
                dataType: 'json',
                async: false,
                cache: false,
                success: function (data) {
...
function DeleteCandidateRoad(urlparameter) {
    if (confirm("Are you sure you want to delete DRRP Details?")) {
        $.ajax({
            type: 'POST',
            url: '/CoreNetWork/DeleteMappedDRRPDetailsVVP/' + urlparameter,
            dataType: 'json',
            async: false,
            cache: false,
            success: function (data) {
