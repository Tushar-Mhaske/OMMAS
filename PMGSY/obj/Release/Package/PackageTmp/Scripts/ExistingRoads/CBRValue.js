$(document).ready(function () {
    ...
    $('#btnSave').click(function (evt) {
        ...
        if ($('#frmCBRValue').valid()) {
            ...
            if (calculateSegmentLenght() && SegmentLenghtValidation()) {
                $.ajax({
                    url: "/ExistingRoads/AddCBRDetails/",
                    type: "POST",
                    cache: false,
    ...
    $('#btnUpdate').click(function (evt) {
        ...
        if ($('#frmCBRValue').valid()) {
                ...
                $.ajax({
                    url: "/ExistingRoads/EditCBRDetails/",
                    type: "POST",
                    cache: false,
                    data: $("#frmCBRValue").serialize(),
                    beforeSend: function () {
                ...
                $('#accordion').show('fold', function () {
                    ...
                    $("#divExistingRoadsForm").load('/ExistingRoads/ShowAddCBRDetails/' + urlparamater, function () {
                        $.validator.unobtrusive.parse($('#frmCBRValue'));
    ...
    $('#btnCancel').click(function () {
        ...
        $('#accordion').show('fold', function () {
            ...
            $("#divExistingRoadsForm").load('/ExistingRoads/CBRCancel/' + MAST_ER_ROAD_CODE, function () {
...
function EditCBR(key) {
    ...
    $('#accordion').show('fold', function () {
        blockPage();
        $("#divCBRDetails").load('/ExistingRoads/EditCBRDetails/' + key, function () {
            $.validator.unobtrusive.parse($('#frmCBRValue'));
            if ($("#Operation").val() == "U") {
...
function DeleteCBR(key) {
    ...
    if (confirm("Are you sure you want to delete the CBR details ? ")) {
        ...
        $.ajax({
            url: "/ExistingRoads/DeleteCBRDetails/"+key,
            type: "POST",
            cache: false,          
            beforeSend: function () {
                blockPage();
...
function DisplayTotalRemainingRoadLength(MAST_ER_ROAD_CODE) {
    ...
    $.ajax({
        url: '/ExistingRoads/CBRlengthCalculation/',
        type: 'POST',
        beforeSend: function () {
            blockPage();
        },
