...
$(document).ready(function () {
    ...
    $("#btnSearch").click(function ()
    {
        ...
        var distcode = $("#ddlDistrictsSearch").val();
     ...
     //   alert("Code" + distcode)
        ...
        if ($("#ddlBlocksSearch").val() == "-1") {
            alert("Please select Block")
            return;
        }

        if ($("#ddlHabitationSearch").val() == "-1") {
    ...
    $("#ddlDistrictsSearch").change(function () {
        loadBlock($("#StateCode").val(), $("#ddlDistrictsSearch").val());

    });


    $("#spnBlockFinalizedErrorMsg").hide();

    $("#ddlBlocksSearch").change(function () {
        $("#ddlBlocksSearch").addClass("valid");
        var BlockCode = $('#ddlBlocksSearch option:selected').val();
        $.ajax({
            url: '/LocationMasterDataEntry/GetHabitationBlockCodeSearchDefinalize/',
            ...
            success: function (data) {
                ...
                console.log(data.isfinalzedval)
                if (data.isfinalzedval)
                {
                    $("#spnBlockFinalizedErrorMsg").show();
                }
                else
                {
                    $("#spnBlockFinalizedErrorMsg").hide();
...
function loadBlock(stateCode, districtCode) {
    $("#ddlBlocksSearch").val(0);
    $("#ddlBlocksSearch").empty();

    if (districtCode > 0) {
        if ($("#ddlBlocksSearch").length > 0) {
            $.ajax({
                url: '/LocationMasterDataEntry/BlockDetailsForDefinalization',
                type: 'POST',
                data: { "StateCode": stateCode, "DistrictCode": districtCode },
                success: function (jsonData) {
                    for (var i = 0; i < jsonData.length; i++) {
                        $("#ddlBlocksSearch").append("<option value='" + jsonData[i].Value + "'>" + jsonData[i].Text + "</option>");
                    }

                    if ($("#MAST_BLOCK_CODE").val() > 0) {
                        $("#ddlBlocksSearch").val($("#MAST_BLOCK_CODE").val());
        ...
        $("#ddlBlocksSearch").append("<option value='0'>All Blocks</option>");
    }
}
...
function LoadGrid() {
    ...
    jQuery("#tbPanchyatList").jqGrid({
        url: '/LocationMasterDataEntry/GetFacilityDetailsListForDefinalization',
        datatype: "json",
        mtype: "POST",
        colNames: ['Facility ID', 'Facility Category ID', 'District Name ', 'Block Name', 'Habitation Name', 'Facility Category', 'Facility type', 'Facility Name', 'Address', 'Pin Code', "PhotoGraph", 'Finalize', 'IsBlockFinalized', 'Upload Photograph',
            'Edit', 'Delete', 'Finalize', 'View Details', 'Definalize &nbsp;&nbsp; Select All &nbsp;&nbsp;<input id="selectAll" type="checkbox" name="AllElements" value="SelectAll"/>'],
        colModel: [
                             ...
                             { name: 'IsBlockFinalize', index: 'IsBlockFinalize', width: 140, sortable: false, resize: false, align: "center", hidden: true },
                             { name: 'UploadPhoto', index: 'UploadPhoto', width: 45, sortable: false, resize: false, align: "center", hidden: true },
                             { name: 'ActionEdit', index: 'ActionEdit', width: 40, sortable: false, resize: false, align: "center", hidden: true },
                             { name: 'Action', index: 'Action', width: 40, sortable: false, resize: false, align: "center", hidden: true },

                             { name: 'FinalizeDetails', index: 'FinalizeDetails', width: 40, sortable: false, resize: false, align: "center", hidden: true },
                             { name: 'ActionView', index: 'Action', width: 40, sortable: false, resize: false, align: "center" },
                             { name: 'DeFinalizeDetails', index: 'DeFinalizeDetails', width: 60, sortable: false, resize: false, align: "center" },
       ...
       // loadonce: true,
        ...
        loadComplete: function () {
            ...
            $('#selectAll').parent().removeClass('ui-jqgrid-sortable');//to make header checkbox clickable
             $("#dvPanchyatListPager_left").html("<input type='button' style='margin-left:27px;margin-top:2px;display:none' id='definalizedAll' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only'title='DeFinalize All Facilities Details' onClick ='definalizeAllFacilities()' value='Definalize All'/>");
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                var isBlockFinalized = $('#tbPanchyatList').jqGrid('getCell', cl, 'IsBlockFinalize');
                if (IsFinalized === "Yes" && isBlockFinalized == "False") {
                    ...
                    DeFinalize = "<center><span class='ui-icon ui-icon-circle-plus' title='DeFinalize Facility Details' onClick ='definalizeFacility(\"" + EncryptedID + "\");'></span></center>";
                    notAllDefinalized = true;
                }
                else if (IsFinalized === "-") {
                    be = "<center><span class='ui-icon ui-icon-locked' title='Locked' onClick ='abc(\"" + cl + "\");'></span></center>";
                    edit = "<center><span class='ui-icon ui-icon-locked' title='Locked' onClick ='abc(\"" + cl + "\");'></span></center>";
...
function definalizeFacility(id) {
    debugger;
    if (confirm("Are you sure to Definalize this facility details ?")) {
        debugger;
        $.ajax({
...
// Definalize ALL
function definalizeAllFacilities() {
    debugger;
    if (confirm("Are you sure to Definalize All facilities details ?")) {
        debugger;
        var encyptedIds = [];
         var ids = jQuery("#tbPanchyatList").jqGrid('getDataIDs');
       // var ids = $("#tbPanchyatList").jqGrid('getGridParam', '_index');
    //    var mydata = $("#tbPanchyatList").jqGrid("getGridParam", "data"),
    //ids = $.map(mydata, function (item) { return item.FacilityID; });
        for (var i = 0; i < ids.length; i++) {
            cl = ids[i];
            var IsFinalized = $('#tbPanchyatList').jqGrid('getCell', cl, 'Finalize');
            var EncryptedID = $('#tbPanchyatList').jqGrid('getCell', cl, 'FacilityID');
            var isBlockFinalized = $('#tbPanchyatList').jqGrid('getCell', cl, 'IsBlockFinalize');
            if (IsFinalized === "Yes" && isBlockFinalized == "False") {
                encyptedIds.push(EncryptedID);
            }
        }
        var postData = { values: encyptedIds };
       
        $.ajax({
            url: '/LocationMasterDataEntry/DefinalizeFacilityForAll/',
            data: postData,
            datatype: "json",
            traditional: true,
            type: "POST",
            ...
            success: function (response) {
                if (response.Success) {
                    alert("All Facilities Details Definalized successfully");
                    //$('#divDisplayFacilityDetails').hide("slow");
                    //$('#dvPanchyatList').show();
