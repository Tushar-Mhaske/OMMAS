//Ims Riding Quality Length
$.validator.unobtrusive.adapters.add('isvalidrangeofridingqualitylength', ['imsrqlength'], function (options) {
    options.rules['isvalidrangeofridingqualitylength'] = options.params;
    options.messages['isvalidrangeofridingqualitylength'] = options.message;
});

$.validator.addMethod("isvalidrangeofridingqualitylength", function (value, element, params) {
    //alert(params.imsrqlength);
    if (parseFloat(value) > parseFloat($('#IMS_PAV_LENGTH').val()))
        return false;
    return true;
});
...
$(document).ready(function () {
    ...
    //$('#txtRidingQualityLength').blur(function () {
    //    if (parseFloat($('#ImsRidingQualityLength').val()) > parseFloat($('#IMS_PAV_LENGTH').val()))
    //    {
    //        alert('Riding Quality Length should be less than or equal to Pavement length');
    //    }
    //});
    ...
    $("#MAST_BLOCK_CODE").change(function () {
        ...
        if ($("#MAST_BLOCK_CODE").val() > 0) {
                ...
                if (Boolean(proceedExecutionFurther)) {
                    ...
                    if (!$("#rdoStageII").is(":checked")) {
                        ...
                        $.ajax({
                            ...
                            data: { Batch: $("#IMS_BATCH").val(), BlockID: $("#MAST_BLOCK_CODE").val(), IMS_UPGRADE_CONNECT: $('input:radio[name=IMS_UPGRADE_CONNECT]:checked').val(), PROPOSAL_TYPE: 'P', value: Math.random() },
            ...
            $("#PLAN_CN_ROAD_CODE").empty();
            if ($("#PMGSYScheme").val() == "4") {
                $("#PLAN_CN_ROAD_CODE").append("<option value=''>Select TR/MRL Number</option>");
            }
            else {
    ...
    $("#IMS_COLLABORATION").change(function () {
        //if ($("#hdnOperation").val() != "U")
        {
            ...
            if ($("#IMS_COLLABORATION option:selected").val() == 5) {
                ...
                $.ajax({
                    url: '/Proposal/PopulateLinkThroughListPMGSY3?blockCode=' + $("#MAST_BLOCK_CODE").val(),
                    type: 'POST',
                    beforeSend: function () {
                        blockPage();
                    },
        ...
        if ($('#frmCreateProposal').valid()) {
            if (validate()) {
                ...
                if ($("#hdnOperation").val() == "C") {
                    ...
                    $.ajax({
                        url: '/Proposal/CreateProposalPMGSY3',
                        type: "POST",
                        cache: false,
                ...
                if ($("#hdnOperation").val() == "U") {
                    ...
                    $.ajax({
                        url: '/Proposal/EditProposalPMGSY3',
                        type: "POST",
                        cache: false,
                        data: $("#frmCreateProposal").serialize(),
                        beforeSend: function () {
    ...
    $("#IMS_BATCH").change(function () {
        ...
        if ($("#MAST_BLOCK_CODE").val() > 0) {
            $("#MAST_BLOCK_CODE").trigger('change');
    ...
    if ($('#PMGSYScheme').val() == 4) {  //Added by Aditi on 11 Aug 2020
        if ($('#txtBrickSolling').val() >=0  && $('#txtBT').val() >=0 && $('#txtCC').val() >=0 && $('#txtGravel').val() >=0 && $('#txtMoorum').val() >=0 && $('#txtTrack ').val() >=0 && $('#txtWBM ').val() >=0) {
            $('#existingSurfaceDw').prop('disabled', true);
            //$('#existingSurfaceDw').val('1');
        }
    }
    ...
    $('#IMS_SANCTIONED_CD_AMT,#IMS_SANCTIONED_PW_AMT,#IMS_SANCTIONED_OW_AMT,#IMS_SANCTIONED_PAV_AMT,#IMS_STATE_SHARE,#IMS_HIGHER_SPECIFICATION_COST,#txtImsPuccaSideDrains').blur(function () {
...
function setCurrentFinancialYear() {
    ...
    // Following two lines are commented on 15 April 2020
...
function CalculateStateShareMordShareTotalCost() {
    ...
    if ($('#PMGSYScheme').val() == 4) {
        ...
        //var TotGSTCost = isNaN(parseFloat($('#txtImsGSTCost').val())) ? parseFloat("0") : parseFloat($('#txtImsGSTCost').val());
