...
function ListQualityFiles(FEED_ID,REP_ID) {
  ...
  //  alert("Hre")
    ...
    jQuery("#tbFilesList").jqGrid({
        url: '/FeedbackDetails/ListPDFFilesByPIU?' + Math.random(),
        datatype: "json",
        mtype: "POST",
...
{
    if ($("#hdnRoleImgUpload").val() == 25 || $("#hdnRoleImgUpload").val() == 8) {
        // For MORD 25 , and For SQC 8 Hide PDF
...
function DownLoadPDF(cellvalue) {
    //alert("cellvalue = " + cellvalue)
    var url = "/FeedbackDetails/DownloadPDFFileUploadedByPIU/" + cellvalue;
...
function DeletePDFFileDetails(cellvalue) {

    if (confirm("Are you sure to delete the PDF and its details ? ")) {

        $.ajax({
            url: "/FeedbackDetails/DeletePDFFileDetailsByPIU/" + cellvalue,
            type: "POST",
            cache: false,
            //data: {
