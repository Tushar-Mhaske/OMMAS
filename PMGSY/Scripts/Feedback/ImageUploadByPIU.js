...
$(document).ready(function () {
    ...
    if ($("#hdnRoleImgUpload").val() == 25 || $("#hdnRoleImgUpload").val() == 8) {
...
function ListQualityFiles(FEED_ID,REP_ID) {
  ...
  //  alert("Hre")
    ...
    jQuery("#tbFilesList").jqGrid({
        url: '/FeedbackDetails/ListFilesByPIU?' + Math.random(),
        datatype: "json",
        mtype: "POST",
...
function isHidden(FinalizedByPIU)
{
    if ($("#hdnRoleImgUpload").val() == 25 || $("#hdnRoleImgUpload").val() == 8)
    {
        // For MORD 25 , and For SQC 8 Hide Image
...
function DownLoadImage(cellvalue) {
    ...
    var url = "/FeedbackDetails/DownloadFileUploadedByPIU/" + cellvalue;
...
function DeleteFileDetails(cellvalue) {
    ...
    if (confirm("Are you sure to delete the Image and Image details ? ")) {
        ...
        $.ajax({
            url: "/FeedbackDetails/DeleteFileDetailsByPIU/" + cellvalue,
            type: "POST",
            cache: false,
            //data: {
