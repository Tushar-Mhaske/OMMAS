$(document).ready(function () {
    ...
    $('#btnSaveHabitation').click(function (e) {
        ...
        if (blockCodes != '') {
            ...
            $('#hdnERRoadCode').val($('#ddlRoadsToMap option:selected').val());

            if (confirm("Are you sure you want to map Habitation Details?")) {
                blockPage();

                $.ajax({
                    url: "/CoreNetWork/MapHabitationsToNetworkRCPLWE",
                    type: "POST",
                    dataType: "json",
...
function LoadMapHabitationList() {
    ...
    jQuery("#mapHabitationList").jqGrid({
        url: '/CoreNetwork/GetHabitationListToMapRCPLWE',
        datatype: "json",
        mtype: "POST",
...
function LoadHabitationGrid() {
    ...
    jQuery("#habitationCategory").jqGrid({
        ...
        colModel: [
                            ...
                            { name: 'PLAN_RD_NAME', index: 'PLAN_RD_NAME', width: 100, sortable: true, align: "center", hidden: true },
...
function LoadScheme2MappedHabitations() {
    jQuery("#tblScheme2Habitations").jqGrid({
        ...
        colNames: ['Name of Habitation', 'Block', 'Village', 'Road Number', 'Total Population', 'Delete'], //'SC/ST Population', 'Primary School', 'Middle Schools', 'High Schools', 'Intermediate Schools', 'Degree College', 'Health Services','Dispensaries'],//,'MCW Centers','PHCS','Vetarnary Hospitals','Telegraph Office','Telephone Connections','Bus Service','Railway Stations','Electricity','Panchayat Head Quarters','Tourist Place'],
        colModel: [
                            { name: 'MAST_HAB_NAME', index: 'MAST_HAB_NAME', width: 400, align: "left", sortable: true },
                            ...
                            { name: 'a', index: 'a', formatter: FormatColumn, width: 50, sortable: true, align: "center", hidden: true },
        ],
