$(document).ready(function () {
    ...
    if ($('#RoleCode').val() == 36 || $('#RoleCode').val() == 47 || $('#RoleCode').val() == 56) {
        LoadProposedRoadsForITNO();
    }
    else {
        LoadProposedRoads();
    }

    //$('#dvAddAgreementAgainstRoad').load('/Agreement/AddAgreementAgainstRoad');
    //$('#dvAddAgreementAgainstRoad').show();
    //blockPage();
    //unblockPage();
...
function LoadProposedRoads() {
    ...
    if ($('#AgreementType').val() == "O") {
        ...
        gridCaption = 'Sanctioned Work List for  Road / LSB / Building';
    ...
    jQuery("#tbProposedRoadList").jqGrid({
        ...
        datatype: "local",
        mtype: "POST",
        colNames: ['Block','Year', 'Batch', 'Package', 'Work', 'Work Type', 'Road Length(In Kms)/ Bridge Length(In Mtrs)', 'Funding Agency', 'Sanctioned Cost', 'Agreement Cost','Maintenance Cost', 'View'],
        colModel: [
...
function LoadProposedRoadsForITNO() {
    ...
    if ($('#AgreementType').val() == "O") {
        ...
        gridCaption = 'Sanctioned Work List for Other Road / LSB / Building';
    ...
    jQuery("#tbProposedRoadList").jqGrid({
        url: '/Agreement/GetProposedRoadListForITNO',
        datatype: "local",
        mtype: "POST",
        colNames: ['Block', 'Year', 'Batch', 'Package', 'Work', 'Work Type', 'Road Length(In Kms)/ Bridge Length(In Mtrs)', 'Funding Agency', 'Sanctioned Cost', 'Agreement Cost', 'Maintenance Cost', 'View'],
        colModel: [
       ...
       // type: "POST",
        ...
        success: function (data) { 
            if (data.isSplitWorkFinalized == true) {
                $.ajax({
                    url: "/Agreement/AddAgreementAgainstRoad/" + parameter,
                    type: "GET",
                    async: false,
                    cache: false,
                    ...
                    success: function (data) {
                        ...
                        $("#dvAddAgreementAgainstRoad").html(data);
                        $('#accordion').show('slow');
                        $('#dvAddAgreementAgainstRoad').show('slow');
