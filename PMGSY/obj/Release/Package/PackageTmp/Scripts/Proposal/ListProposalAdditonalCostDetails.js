$(document).ready(function () {
    ...
    $.validator.unobtrusive.parse('#searchPropAddCost');
    ...
    //LoadProposalAdditionalCostGrid();

    //list button click
    $("#btnListPropAddCost").click(function () {
...
function LoadProposalAdditionalCostGrid() {

    jQuery("#tbPropExecutionList").jqGrid({
        url: '/Proposal/GetProposalAdditionalCostList',
        datatype: "json",
        mtype: "POST",
        ...
        colNames: ['State','District','Block', 'Year', 'Batch', 'Package No.', 'Road Name', 'Road/LSB Cost (In Lacs)', 'Road Length(in Kms)/LSB Length(int Mtrs)', 'Maintenance Cost(in Lacs)', 'Additional Cost'],
        colModel: [
        ...
        pager: jQuery('#pagerPropAddCost').width(20),
        rowNum: 10,
        rowList: [10, 20, 30],
        viewrecords: true,
        ...
        sortorder: "asc",
        caption: "&nbsp;&nbsp; Proposal Additiona Cost Details List",
        height: 'auto',
        autowidth: true,
        hidegrid: true,
        rownumbers: true,
...
function AddAdditionalCostDetail(urlparameter) {

    $("#accordion div").html("");
    $("#accordion h3").html(
            "<a href='#' style= 'font-size:.9em;' >Add Additional Cost Details</a>" +
            '<a href="#" style="float: right;">' +
            '<img class="ui-icon ui-icon-closethick" onclick="ClosePropAddCostDetails();" /></a>'
            );

    $('#accordion').show('fold', function () {
        blockPage();
        $("#divAddPropAddCost").load('/Proposal/AdditionalCostDetails?id=' + urlparameter, function () {
            //$.validator.unobtrusive.parse($('#frmAddPhysicalRoad'));
            unblockPage();
        });
        $('#divAddPropAddCost').show('slow');
        $("#divAddPropAddCost").css('height', 'auto');
    });
...
function ClosePropAddCostDetails() {

    $("#accordion").hide('slow');
    $("#divAddPropAddCost").hide('slow');
