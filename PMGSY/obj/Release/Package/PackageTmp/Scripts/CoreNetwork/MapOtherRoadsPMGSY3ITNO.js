...
$(document).ready(function () {
    ...
    $("#definalize").click(function () {
        if (confirm("Are you sure you want to definalize DRRP Details?")) {
            $.ajax({
                type: 'POST',
                url: '/CoreNetWork/DeFinalizeMappedDRRPDetailsPMGSY3ITNO?id=' + $("#CNCode").val(),
                dataType: 'json',
                data: { __RequestVerificationToken: $("#frmMapDRRP input[name=__RequestVerificationToken]").val() },
                async: false,
                cache: false,
...
function LoadMappedRoadsList(CNCode) {
    ...
    jQuery("#tblListMappedRoads").jqGrid({
        url: '/CoreNetwork/GetMappedCandidateRoadListPMGSY3ITNO',
        datatype: "json",
        mtype: "GET",
        ...
        loadComplete: function (data) {
            console.log(data.IsFinalized);
            if (data.IsFinalized == "Y") {
                $("#tblDeFinalize").show();
                $("#tblFinalize").hide();
                $("#dvMapDRRP").hide();
            }
            else if (data.IsFinalized == "N" || data.IsFinalized == "" || data.IsFinalized == undefined) {
                $("#tblFinalize").show();
                $("#tblDeFinalize").hide();
                $("#notFinalizedMsg").show();
                $("#isPCIEntryMsg").hide();
            }
            else if (data.IsFinalized == "PCI" || data.IsFinalized == "" || data.IsFinalized == undefined) {
                $("#tblFinalize").show();
                $("#tblDeFinalize").hide();
                $("#notFinalizedMsg").hide();
                $("#isPCIEntryMsg").show();
