$(document).ready(function () {
    ...
    $('#ddlRole').change(function () {
        if (parseInt($('#ddlRole option:selected').val()) == 54 || parseInt($('#ddlRole option:selected').val()) == 55 || parseInt($('#ddlRole option:selected').val()) == 0)
        {
            ...
            $("#ddlSchemeSearch").append("<option value='4'>PMGSY-3</option>");
        ...
        switch (option) {
            ...
            case "R":
                ...
                switch (module) {
                    ...
                    // Added by Shreyas to unlock C-Proforma PDF
                    case '11':
                        $("#divFilterForm").show();
                        $("#lblState").show('slow');
                        $("#ddlStateSearch").show('slow');
            ...
            case "Y":
                ...
                // Old Code commented on 04-01-2023
                //if ($("#ddlModule option:selected").val() == 2 || $("#ddlModule option:selected").val() == 8) {
                //    $("#lblCollaboration").show('slow');
                //    $("#ddlCollaboration").show('slow');
                //}
                ...
                // New Code added on 04-01-2023
                if ($("#ddlModule option:selected").val() == 2 || $("#ddlModule option:selected").val() == 8 || $("#ddlModule option:selected").val() == 10 || $("#ddlModule option:selected").val() == 11) { // Added val 11 by Shreyas for to unlock C-Proforma PDF
                    $("#lblCollaboration").show('slow');
                    $("#ddlCollaboration").show('slow');
                }
                break;
...
function AddUnlockDetails() {
    ...
    switch (level) {
        ...
        case 'R':
            ...
            switch (module) {
                ...
                // Added By to unlock C-Proforma PDF
                case '11':
    ...
    $('#accordion').show('fold', function () {
        ...
        //------START Added to unlock Proposal Technology Details
        ...
        // New Code to change in Yearbatch param batch,package added
        $.post('/LockUnlock/AddEditUnlockDetails/', { moduleCode: $("#ddlModule option:selected").val(), levelCode: level, unlockData: unlockData, scheme: $("#ddlSchemeSearch option:selected").val(), yearbatch: $('#ddlStateSearch option:selected').val() + '$' + $('#ddlDistrictSearch option:selected').val() + '$' + $('#ddlBlockSearch option:selected').val() + '$' + $('#ddlYearSearch option:selected').val() + '$' + $('#ddlBatchSearch option:selected').val() + '$' + $('#ddlPackageSearch option:selected').val(), roleCode: $('#ddlRole option:selected').val() }, function (data) {
            $("#divAddUnlockDetails").html(data);
            $.validator.unobtrusive.parse($('#divAddLockDetailsForm'));
            unblockPage();
        });

        //------END Added to unlock Proposal Technology Details
...
function CloseUnlockDetails() {
    ...
    switch (level) {
        ...
        // Added to unlock Proposal Technology Details
        case 'Y':
            $("#tbPropYears").jqGrid('setGridState', 'visible');
            break;
        case 'T':
            $("#tbPropBatches").jqGrid('setGridState', 'visible');
            break;
    }
    $("#accordion").hide('slow');
}
...
function ViewDetailsToUnlock(urlparam) {
    ...
    switch (module) {
        ...
        case "8":
            ...
            break;
        case "10":
            ViewProposalDetails(urlparam, "PT");        // Added to unlock Proposal Technology Details
            HideAllDetails();
            $("#dvProposalDetails").show();
            break;
        case "11":
            ViewProposalDetails(urlparam, "CP");        // Added to unlock C-Performa PDF
            HideAllDetails();
            $("#dvProposalDetails").show();
            break;
    }
