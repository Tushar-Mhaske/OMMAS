...
function districtwiseScheduleDetails() {
    ...
    jQuery("#tbDistrictwiseSchList").jqGrid({
        ...
        colNames: ["Select", "District", "Block", "Package", "Sanction Year", "Road Name", "Type", "Stage", "Length (Road-Km / LSB-Mtr)", "Contractor Name (Work Award Date)", "Work Status (Current)", "Work Status (As on schedule)", "Enquiry Inspection",
        ...
        colModel: [
            ...
            { name: 'Stage', index: 'Stage', width: 60, sortable: false, align: "left", search: false },
            ...
            { name: 'SchdRdStatus', index: 'SchdRdStatus ', width: 130, sortable: false, align: "left", search: false },
            ...
            { name: 'Finalize', index: 'Finalize', width: 60, sortable: false, align: "center", search: false, hidden: true },
        ...
        loadComplete: function () {
            ...
            if ($("#hdnRoleCodeDistwiseSch").val() == 8) //for SQC only show Finalize All button
            {
                if ($("#INSP_STATUS_FLAG").val() != "UPGF") {       //If Schedule not forwared to monitor then only show this button
                    ...
                    if (recordCnt > 0 && $('#tbDistrictwiseSchList').jqGrid('getGridParam', 'records')) {
                        $("#tbDistrictwiseSchList #dvDistrictwiseSchListPager").css({ height: '35px' });
                        $("#dvDistrictwiseSchListPager_left").html("<input type='button' style='margin-left:27px;margin-top:2px; color:red;' id='btndeleteall' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onClick = 'GetDataInArray();return false;' value='Delete'/>");
                        $("#dvDistrictwiseSchListPager_center").html("<input type='button' style='margin-left:1px; color:darkgreen; font-size:14px' id='idFinalizeAllRoads' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onClick = 'FinalizeSchRoad(0, true);return false;' value='Finalize All'/>");
                    }
                    else if (recordCnt > 0) {
                        $("#tbDistrictwiseSchList #dvDistrictwiseSchListPager").css({ height: '35px' });
                        $("#dvDistrictwiseSchListPager_center").html("<input type='button' style='margin-left:1px; color:darkgreen; font-size:14px' id='idFinalizeAllRoads' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ' onClick = 'FinalizeSchRoad(0, true);return false;' value='Finalize All'/>");
                    }
                    else if ($('#tbDistrictwiseSchList').jqGrid('getGridParam', 'records')) {
                        $("#dvDistrictwiseSchListPager_left").html("<input type='button' style='margin-left:27px;margin-top:2px; color:red;' id='btndeleteall' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onClick = 'GetDataInArray();return false;' value='Delete'/>");
                    }
                }
            ...
            if ($("#hdnRoleCodeDistwiseSch").val() == 9 || $("#hdnRoleCodeDistwiseSch").val() == 5) {
            ...
            //}
            else if ($("#hdnRoleCodeDistwiseSch").val() == 8) {
                $("#dvNoteTab").html("<h3 style='color:green; text-align:center'>Note:  <i>'Finalize All'</i> will finalize all works in the Schedule</h3>")
            }
    ...
    $("#tbDistrictwiseSchList").jqGrid("setLabel", "cb", "Delete");
