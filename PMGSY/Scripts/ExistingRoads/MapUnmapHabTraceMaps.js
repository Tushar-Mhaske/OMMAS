...
function LoadGrid() {
    ...
    jQuery("#tblTraceMapList").jqGrid({
        url: '/ExistingRoads/MapUnmapHabGetDistrictList',
        datatype: "json",
        mtype: "POST",
        colNames: ["Distict Name", "Block Name", 'Download Facility List', GetColNamePDF(), GetColNameCSV(), GetColNameHabCSV(),"Map Habitations From CSV", GetFinalizeStatus(), "Definalize"],
        colModel: [
                             ...
                             { name: 'DownloadExcel', index: 'DownloadExcel', height: 'auto', width: 120, align: "center", hidden: true },
                             { name: 'UploadPDF', index: 'UploadPDF', height: 'auto', width: 120, align: "center", hidden: true },
                             { name: 'UploadCSV', index: 'UploadCSV', height: 'auto', width: 120, align: "center", hidden: true },

                             { name: 'UploadHabCSV', index: 'UploadHabCSV', height: 'auto', width: 120, align: "center", hidden: false }, // UploadHabCSV()
                             { name: 'MapUnmapHab', index: 'MapUnmapHab', height: 'auto', width: 120, align: "center", hidden: false }, // MapUnmapHabDetails

                             { name: 'Finalize', index: 'Finalize', height: 'auto', width: 120, align: "center", hidden: true },
                             { name: 'Definalize', index: 'Finalize', height: 'auto', width: 120, align: "center", hidden: true },
        ],
...
function MapUnmapHabDetails(id) {

    if (confirm("Are you sure to map Habitations from CSV and delete already mapped Habitations ? ")) {

        $.ajax({
            url: "/ExistingRoads/MapUnmapHabitationDetails/",
            type: "POST",
            cache: false,
            data: { BlockCode: id },
            beforeSend: function () {
            ...
            success: function (response) {
                ...
                if (response.success) {
                    //    $("#NumberofFiles").val(parseFloat($("#NumberofFiles").val()) - 1);
                    alert("Already mapped Habitations are deleted and Habitations from CSV are mapped Successfully.");
                ...
                else
                {
                    ...
                    // alert("Habitation CSV file must be uploaded by ITNO and finalized by PIU against this Block.");
                }
            }
        });
