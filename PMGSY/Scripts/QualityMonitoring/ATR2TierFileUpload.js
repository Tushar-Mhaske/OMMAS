...
function CloseObservationDetails() {
    $('#accordionATR2selectedTierCqc').hide('slow');
}
 
function Show2TierSQCATRSelectedObsDetails(obsId) {
    ...
    $("#accordionATR2selectedTierCqc div").html("");
    $("#accordionATR2selectedTierCqc h3").html(
        "<a href='#' style= 'font-size:.9em;' >Marked SQM Observation Details</a>" +
        '<a href="#" style="float: right;">' 
        //'<img  style="border:none;border-width: 0; " class="ui-icon ui-icon-closethick" style="border-width: 0;" onclick="CloseObservationDetails();" /></a>' +
        //'<span style="float: right;"></span>'
    );

    $('#accordionATR2selectedTierCqc').show('slow', function () {
        blockPage();
        $("#dvATR_S_selectedWork").load('/QualityMonitoring/QMATRAccpetedObsDetails/' + obsId + '/false', function () {
            unblockPage();
        });
    });
    $("#dvATR_S_selectedWork").css('height', 'auto');
    $('#dvATR_S_selectedWork').show('slow');
...
function List2TierPDFFiles(QM_OBSERVATION_ID) {

    //  alert("inside ListPDFFiles");
    blockPage();
    jQuery("#tbPDFFilesList").jqGrid({
        url: '/QualityMonitoring/List2TierPDFFiles',
        datatype: "json",
        mtype: "POST",
        colNames: ["ATR File"],
        colModel: [
            { name: 'ViewPDF', index: 'ViewPDF', width: 400, align: "center", search: false, sortable: false,  resizable: true },
...
function viewATR2PdfVirtualDir(id) {
   


    $.ajax({
        type: "POST",
        url: '/QualityMonitoring/viewATR2PdfVirtualDir/' + id,
        dataType: 'json',
        contentType: false,
        processData: false,
        cache: false,
...
function AnchorFormatter(cellvalue, options, rowObject) {
     var url = "/QualityMonitoring/Download2TierATRFile/" + cellvalue;
...
/*Added on 30-01-2023*/
function ViewVerificationATR_ListByRoadCode_Inspdate_ObsId_ATRId_2Tier(roadCode_inspdate_ObsId_ATRId) {
  
  //  alert("ViewVerificationATR_ListByRoadCode_Inspdate_ObsId_ATRId_2Tier");
    //passing NQM Observation ID and ATR Id for finalize
    var urltemp = "";
    const SRIorU_SQM_ObsId_ATrCode = roadCode_inspdate_ObsId_ATRId;//.split("$");
   
    if ($("#allowRejectedAtrView").val()=="True") {
       
        urltemp = '/QualityMonitoring/ViewVerificationRejectedATR_ListByRoadCode_Inspdate_ObsId_ATRId_2Tier';
    }
    else {
        urltemp = '/QualityMonitoring/ViewVerificationATR_ListByRoadCode_Inspdate_ObsId_ATRId_2Tier';
     }
    ...
    jQuery("#tblVerificationATR_Grid").jqGrid({
        ...
        postData: { "VerificationATRCode": SRIorU_SQM_ObsId_ATrCode },
        ...
        loadComplete: function (data) {
            ...
            if (data["records"] > 0 && userdata.ids.length == 0 && userdata.ids.length < 1) {
                ...
                $('#divPagerVerificationATR_Grid_left').html("<input type='button' style='margin-left:50px; border: 2px outset green;' id='btnFinalizedVerifyATR' class='jqueryButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only' onClick ='finalizedVerification2TierATR(\"" + SRIorU_SQM_ObsId_ATrCode + "\"); return false;' value='Finalize'/>");
            }
            unblockPage();
        },
...
function finalizedVerification2TierATR(SRIorU_SQM_ObsId_ATrCode) {
    ...
    if (userdata.ids.length == 1) {
        ...
        if (selRowIds.length > 0) {
            ...
            if (selRowIds.length == 1) {
                ...
                savefinalizedMarkVerification2TierATR(SRIorU_SQM_ObsId_ATrCode, submitArray);
...
function savefinalizedMarkVerification2TierATR(SRIorU_SQM_ObsId_ATrCode, submitArray) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success == true) {
                    ...
                    $.blockUI();
                    viewATRDetails();//use to reload grid
                    $.unblockUI();
...
//            //}
//            //alert("1");
//            alert("SRI or U inspected sqm obs id: " + $(QM_OBSERVATION_ID).val());
//            ViewVerificationATR_ListByRoadCode_Inspdate_ObsId_ATRId_2Tier($(IMS_PR_ROAD_CODE).val() + "$" + $(QM_INSPECTION_DATE).val() + "$" + $(QM_OBSERVATION_ID).val() + "$" + $(QM_ATR_ID).val());
//            List2TierPDFFiles($(QM_OBSERVATION_ID).val());
//            unblockPage();
//        });
